...
import { Button, Glass, StatCoin, Text, useReduceMotion } from '../../ui/index.ts';
import { claimModal, releaseModal } from '../../ui/modal-owner.ts';
import { tw } from '../../ui/tailwind.ts';
import { colors, font, glass, ramp, space } from '../../theme.ts';
...
export function PerchBirdSheet(
  { member, mode = 'current', onClose, onWhack, showWhackHint = false, dark = false, dayLine }: {
    member: PerchMember;
          ...
          <ScrollView style={{ flexGrow: 0, flexShrink: 1 }}>
            <View accessibilityViewIsModal style={tw.style('p-lg gap-md', { width: sheetWidth })}>
              <View style={tw`items-center gap-xs`}>
                ...
                <Text style={{ ...font.body.body, color: dark ? ramp.sage[300] : colors.subtle }}>
                  Level {member.level}
                </Text>
              </View>
              <View
              ...
              >
                {CORE_STATS.map((stat) => (
                  <StatCoin key={stat} stat={stat} points={member.ratings[stat] ?? 0} />
                ))}
              </View>
              ...
              {error && (
                <Text
                  ...
                  style={{ ...font.body.body, color: dark ? colors.coralTint : colors.damage }}
                >
                  {error}
                </Text>
              )}
