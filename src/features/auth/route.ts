...
export function redirectTarget(input: {
  ...
  switch (input.route) {
    ...
    case 'ready':
      ...
      // routes outside any group — `/train`, `/settings`, `/progress` — are
      // legitimate destinations for a signed-in user, and an allowlist of one
      // bounced them straight back to the home tab the instant they were
      // pushed.
