...
- [ ] **Task 8 — `background.ts`.** `configureBackgroundTypes(KAIRO_BACKGROUND_TYPES, UpdateFrequency.hourly)` called once after permission. **Written, not verified.** Leave the roadmap bullet ⬜ with that reason.
