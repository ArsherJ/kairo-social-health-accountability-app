...
| **Alternate-species art or mechanics** — a picker, per-species evolution art, skins, battle frames, a *mechanical* affinity bonus, or animation beyond React Native `Animated` | Deviation #40's four-species choice is historical: deviation #55 replaced it with one canonical Philippine eagle at the render boundary, and deviation #73 gives that eagle one static plush body. Alternate art would split the identity again; a mechanical affinity would rescore replayed history. No new motion dependency is installed: `react-native-svg`, Rive and Reanimated remain out. | Deviations #40, #55, #73; spec §13 |
...
| **Alternate or unlockable species** | The current emotional anchor is one canonical Philippine eagle, not an onboarding species choice. An unlockable roster would make that identity provisional and reintroduce the choice deviation #55 deliberately removed; historical `profiles.species` values remain reversible plumbing only. | Parent spec §11, deviations #40, #55 |
