...
| 60 | The Digest reaches everybody who has a reason to be notified (§14, deviation #52) | **The notification ask opens to solo players and the Digest closes on lapsed ones**, in one change. The ask gains the account's own **first scored day** as a third reason beside a squad and a live Battle; `users_needing_digest()` gains an activity predicate and stops sending to an account with **no scored day in seven local days**, resuming by itself on the next one. | Reachability, not design. Deviation #52 reduced three pushes a day to one and rewrote what the push *says* — including a solo branch in `digestCopy()` written with care to speak no rank — and nobody revisited **who could receive it**. The ask fired only for a squad or a live Battle, so in a solo-first product where most new accounts are alone for their first days, the one scheduled push aimed at the loop reached nobody the loop was rebuilt for. **The two halves are one row because shipping either alone is worse than shipping neither.** Opening the ask without the suppression gives a lapsed solo player a push every morning they would never previously have received — thirty a month to somebody who left, which is how a channel gets switched off permanently, costing it for the moment they might have come back. Applying the suppression without opening the ask changes nothing for the population it was written for, because they hold no token. Five things are worth recording. **(a) The first scored day is the boundary both rules meet at, from opposite sides.** It is the earliest moment the Digest has anything true to report and supplies a why the player can see rather than one borrowed from a squad; it is also why an account that has *never* scored is suppressed rather than exempted — such an account holds no push token at all. That reads like a bug that eats somebody's first Digest, so the reasoning is in the function body: a young-account exemption would be a redundant second rule, and two rules is how they later disagree. **(b) The window is exactly seven local days ending today** — a day scored six days ago qualifies, seven and eight do not — and the boundary is what a future edit is most likely to move by one, so the schema suite pins both sides of it. `total > 0` is the same reading of "scored" every other surface uses, because `sync-health` writes a row per date in the payload whether or not it scored. **(c) A *lapse* is not a *quiet week*, and the glossary now defines the two against each other.** A player who is here and scoring little scores, so they pass the predicate every day; "quiet" was already used twice protectively in `CONTEXT.md`, and using it a third time adversarially would let a future reader read this as punishing the player the product exists for. Lapse stops the Digest and nothing else — no demotion, no lost Mastery, no altered gate, nothing stored and nothing to reset, and the player is never told. **(d) The seven is a commented literal in SQL.** It cannot import from the keystone and a mirrored TypeScript constant would have no reader, because no client ever asks whether it is suppressed — deliberately unlike `DAILY_STEP_BASELINE`, which is derived precisely because two places read it. Suppression is not separately recorded either: `notification_log` already says who was sent, the suppressed population is derivable from scores whenever the question is asked, and `kairo_retention()` is where that question belongs. **(e) No Edge Function redeploys.** The function is replaced in place and its signature does not move, so `dispatch-notifications` keeps calling it by name. **Two ordering constraints, and both windows are silent.** The **migration is applied before the client ships**, or solo players begin holding tokens against an unsuppressed Digest; applied first, nothing changes for anybody until the client follows. And the **landing page is deployed before the OTA that drops the invite message's clause**, or there is a window in which the privacy claim exists on no surface a non-user can reach — which is the exact failure the rest of this row is correcting, reintroduced by sequencing. Out of scope and deliberately so: no lapsed-player copy branch (writing copy for a cohort not yet observed is premature — the retention function describes lapse first), and no re-engagement push, which if it is earned deserves its own design rather than an exemption inside a mechanism whose payload is a rank announcement. |
