...
| 44 | §5, §6 and §8: the app converts activity into a score, ranks it on a daily leaderboard, and hangs squad goals off it. A character exists (§6) but is a *readout* of the score rather than the thing the product is about | **Your real life powers your character, and your character races your friends.** A pre-launch pivot, decided deliberately: the TestFlight build stops being the thing measured. Five sub-projects — the race (#46, #47), Body · Motion · Mind (#51), the Today tab and quests (#50), Events and the pooled Battle (#45, #48, #49), and the digest plus the level response (#52) | Founder decision 2026-08-25, parent spec `docs/superpowers/specs/2026-08-25-character-race-pivot-design.md`, sub-project specs and plans `2026-08-25-the-race`, `-body-motion-mind`, `-the-today-tab`, `-events-and-battle`, `-digest-and-level-response`. **Recorded here, in the last plan of five, rather than in the first** — a roadmap row claiming a pivot that is four-fifths built is a row that misleads. Three things this deviation is *not*, each easy to assume and each wrong. **No user data was destroyed**: `daily_scores`, XP, the three ability ratings, streaks, species, invite codes and challenge completions all survive untouched, and banked Goal XP is kept by the `closed_at` mechanism in #45 rather than migrated or dropped. **The scoring engine is untouched**: `tierFor`, `shiftedTierFor`, `TIER_POINTS`, `THRESHOLDS`, `computeDailyScore`, the `3 / earnable stats` scaling, `planDay`, `finalizable_days()` and the streak all behave exactly as they did — the race reads raw units *alongside* them (#47's widened projection), never instead of them, which is what makes the pivot a change of surface rather than a rescore of history. And **`kairo_retention()` was deliberately not re-pointed**: it measures whether a `daily_scores` row exists on cohort day + N, and the pivot redefined what the app *shows*, not what counts as an active day — rewriting that denominator would make every measurement taken before 2026-08-25 incomparable to every one after, which is the opposite of what a pivot's instrumentation is for, and it is precisely because the definition held still that every chart can be *split* on the pivot date and mean something. What was genuinely stale is the funnel vocabulary: `goal_created` names a surface that no longer exists and is kept only as a historical value, and four events replace it — `squad_data_consent_granted` (the highest-risk step in the whole pivot, since the race cannot draw a lane for a member who has not agreed), `race_seen` (once per local day, the cohort split the pivot exists to answer), `quest_cleared` (`{ tier }`, never a quest id) and `event_created` (`{ kind, difficulty }`, never the target). The query that asks whether a user who saw a race comes back tomorrow more often than one who did not is in `docs/beta-measurement.md`, stated there as a correlation rather than a trial. |
| 45 | §8: a Goal is a per-member target over a window, measured in `daily_scores.total` and graded N-of-M — "everyone must hit it". `goals`, `goal_participants` and `goal_completions` carry it, and `create_goal()` / `goal_window_scores()` are its API | **Goals are gone from every surface, and the `goals` table is reshaped in place into `challenge_events`.** `goal_participants` → `event_participants`, `goal_completions` → `event_completions`, `goal_id` → `event_id`. `create_goal()`, `abandon_goal()`, `goal_window_scores()` and `can_see_goal()` are dropped; `create_event()`, `abandon_event()`, `event_progress()` and `can_see_event()` replace them. The whole of `src/features/goals/` and both `/goal` routes are deleted and `src/features/events/` replaces them. A new nullable `challenge_events.closed_at` carries every pre-pivot row. Migration `20260828090000_events.sql`, shipped with `finalize-days` | Founder decision 2026-08-25, spec `docs/superpowers/specs/2026-08-25-events-and-battle-design.md`, parent `…/2026-08-25-character-race-pivot-design.md`. Plan 4 of 5. **Reshaped, not dropped and rebuilt.** The table already carried `squad_id`, `created_by`, `title`, `description`, a widenable `metric` check and a validated window ordering — but the expensive things it carried were its **column-level grants** (UPDATE on `title, description` only), its **RLS predicate**, its **XP rollup trigger**, and above all its **erasure triggers**. `profiles_collect_orphaned_*` and the `created_by` SET NULL behaviour are the erasure-critical path, and rebuilding them on a fresh table would mean re-deriving, in a migration, the reasoning deviation-era work already paid for once. `alter table … rename` carries indexes, constraints, policies, triggers and grants across for free; only their **names** stay behind, so all fourteen are renamed explicitly — a constraint reading `goals_*` on a table called `challenge_events` is exactly how the next reader concludes the rename was half-done. **`closed_at` is the refinement the parent spec needed, because two of its own sentences conflict.** §9 says live goal rows are *closed out* and that banked `goal_completions` XP *stays, so nobody's level drops*. Both cannot hold under a table rename: a completion's foreign key holds its goal row alive, and that row's `kind` is `cumulative` and its `metric` is `daily_score`, both of which the new checks reject. Deleting the goal rows cascades the completions and drops somebody's level; `add constraint … not valid` leaves a permanently unvalidated constraint that the next reader will try to `VALIDATE` and break. So the table gains `closed_at timestamptz`, every surviving legacy row gets a timestamp, and all four new checks are written `check (closed_at is not null or …)` — a **real, validated** constraint stating exactly the true thing: *a live Event is a Battle or an Adventure; a closed row is whatever it used to be*. The consequence is a rule with no exceptions: **`closed_at is null` is not optional on any read**, and omitting it renders a pre-pivot points goal as a Battle with a four-figure kcal bar. `challenge_events_one_live_per_kind`, the partial unique index enforcing at most one live Battle and one live Adventure per squad, keys off the same column — which is also why `abandon_event()` **closes** rather than deletes: the completion keeps its row and its XP, and the slot frees for the next fight. **Two new constraints the Goal shape did not have.** `events_need_end`, because a Goal could be open-ended ("reach 500,000 points however long it takes" is a coherent commitment) and a boss with no deadline is a slowly filling bar that can never be lost — nothing at stake, no reason to push this week rather than next. And `events_need_squad`, because a personal Battle is a Challenge, which already exists, is already tuned to one person's own history, and is already on `/train`; two mechanics for one thing is how a surface ends up half-built. Both are conditional on `closed_at`, since legacy personal and open-ended goals exist. **Two live-schema facts had to be read rather than assumed, and one of them was a live bug in the plan.** `create_goal` was dropped **by exact argument list** and it had **ten** parameters, not the original migration's eight — `p_description` and `p_metric` were each added by a later drop-and-recreate — while `can_see_goal` had **two**, not one. `create_event` therefore has **no defaulted parameters at all**, deliberately: adding a defaulted parameter to a function that already has defaults is the ambiguous overload PostgREST cannot resolve, and this is the third time that trap has been walked past here. The second fact: `recalculate_user_xp` is a **full recompute written out whole**, and the deployed body names `challenge_completions` as a third XP source *and* writes `agi_total`/`str_total`/`mnd_total`. Only the one table name changed. Writing the function from the two obvious sources would have zeroed every account's stat rollups on the next sync — the failure mode a full recompute has by design, in exchange for never double-counting. **One erasure function had to be recreated rather than renamed**, and the reason generalises: `collect_orphaned_goals()` (the *function* had no `profiles_` prefix; only the trigger did) names `public.goals` and `public.goal_participants` **inside its plpgsql body**, which is text resolved at execution and which `alter table … rename` does not rewrite. A rename alone would have left it raising `relation "public.goals" does not exist` the first time somebody deleted an account — silent until then, and on the one path that must never fail. It stays **AFTER DELETE**: from BEFORE it reaches a completion, which updates `profiles`, which modifies the row being deleted, and Postgres aborts the statement. **`goal_completed` survives as a notification trigger and routes to `/`.** `notification_log.kind` is free `text` with no check constraint, so historical rows already say it and a push sent minutes before the deploy can be tapped minutes after it. Both values stay in the TypeScript union and both route; only new sends use `event_completed`. A historical value matching no case is a tap that goes nowhere, which is indistinguishable from push being broken. **Neither the Battle panel nor `/event/new` carries a disclosure gate**, unlike the goal surfaces they replace. An Event is a squad's shared thing, and gating it on one member's scored-day count would hide from a new member something the rest of the squad is already looking at. The home screen's `core` note was updated in the same pass — it named goals as a thing still to come, and promising a feature that has since become visible reads as the app losing track of itself. |
| 46 | The squad screen is a **daily leaderboard**: rows ordered by the program-weighted score, with the ability ratings beside each name (§7). Nothing in the spec is a race, and the Daily Walk's 10,000 steps is a personal streak target only (deviation #31) | **The board becomes a track.** A Race is an always-on *reading* of a day that already exists — no creation flow, no state, nothing stored: `squad_leaderboard()` for one local date, re-ranked on the client by **capped** steps and drawn as horizontal lanes with the endemic figures running them. `RACE_FINISH_LINE` in `packages/kairo-core/src/race.ts` **is** `DAILY_STEP_BASELINE`, derived and never written as a literal, so crossing the line and clearing the Daily Walk are the same event. The cap **is** the anti-cheat. With no squad, the rivals are the player's own recent days as ghost figures | Founder decision 2026-08-25, spec `docs/superpowers/specs/2026-08-25-the-race-design.md`, parent `…/2026-08-25-character-race-pivot-design.md`. Plan 1 of 5. **A race is not a leaderboard with a different skin, and the difference is what the whole pivot rests on.** A leaderboard ranks; a race has a shared destination, so a lane is a ground rule with a figure standing somewhere along it and the finish line is one continuous rule spanning every lane — six people running at the same flag, which a list of rows structurally cannot show. Drawing it as a filled progress bar would have reintroduced leaderboard vocabulary (a quantity, read left to right) and the redesign would have been cosmetic. **Unifying the finish line with the Daily Walk is the load-bearing decision.** `DAILY_STEP_BASELINE` is `THRESHOLDS.AGI.gold`, and pinning the race there means one number the app teaches, read socially by the race and personally by the streak. A second competitive bar at a different number would split attention between two step targets, in a product whose brief is "simple to understand, deep underneath". Three constraints follow. The race reads **raw steps**, which `daily_scores` does not store — hence the widened projection in #47, never `tiers`. The constant is **imported**, never re-derived: `10_000` appears nowhere in the race code and the plan's definition of done greps for it. And because the race never reads a tier at all, it is clear of the `AGI`/`AGI_base` trap entirely — the spread shift lowers AGI's whole ladder including Gold, so a finish line read out of `tiers->>'AGI'` would move with the user's own active hours, which is precisely the public-health failure the baseline exists to prevent. **The cap is the anti-cheat, and it is not a separate mechanism.** Inside scoring, a 40,000-step day and a 12,000-step day are both Gold, so shaking a phone buys almost nothing; racing on *raw* steps would have handed that resistance straight back. Capping race contribution at the finish line restores it exactly — past the line, extra steps buy nothing at all — with no fraud detection, no threshold to tune and no accusation to make. It also makes the common case correct: two people past the line are tied on the primary key **by construction**, not as an edge case, which is why the tie-break falls through to the daily score (the thing the engine already considers a better day) and then to `user_id` (so the order is stable across refetches and the board does not visibly twitch on every poll). **Two orderings, one payload.** `squad_leaderboard()` keeps ordering by the program-weighted total, because that is the only way a squad's program can apply at read time (#11). The race re-ranks on the client. The obvious "improvement" is to rank once in SQL, and it would silently delete the program feature — so a schema test pins that the returned `rank` sequence is still monotonic in `total`, and pins it on a fixture where the step ordering and the weighted ordering genuinely disagree. **Solo mode races your own past days**, which is the narrow deliberate exception to the source document's §20 warning against solo challenge modes: it exists so nobody meets an empty Squad tab, and so the mechanic teaches itself before a friend arrives. Days that scored nothing are **dropped rather than raced** — a new account otherwise lines up against three zeroes, which reads as the feature being broken rather than as an easy win — and with no qualifying history at all the player runs one lane beside the invite affordance. Never an empty track, never a fabricated rival. Two things there are easy to get wrong: the history query is bounded by a **date range, not a row limit**, because `.limit(14 * 24)` looks equivalent and is not (a day with fewer than 24 stored hours shifts the window and the oldest ghost then races on a fraction of its real steps); and `ghostDayLabel` names a weekday only inside a week, because `ghostRivals` takes the three most recent days that *scored* and a quiet stretch can put two Saturdays on the track. **`race_results` is deliberately not built here** (spec §7.3). A result only needs snapshotting once one is displayed as history, which arrives with the digest; building the table before anything reads it is speculative and would mean touching `finalize-days` twice. Plan 5 carries it under this row rather than claiming a number of its own. **Accessibility was the main build risk and is treated as one.** A six-lane track is the character-HUD failure waiting to happen: the HUD's `+8/+48/+48/+132` constants assumed pill heights nothing enforced and overlapped at large Dynamic Type. Lanes are therefore **flow-based throughout** — the figure is placed by two flex spacers and nothing carries a `top` — and each lane is **one accessibility element with both halves** of the 2026-08-14 grouping fix, since the documented collapse behaviour did not happen on that build. The composition is `race-label.ts`, a pure module tested in Node, extending `row-label.ts` rather than forking it. Where the name column stops fitting beside the track (past ~1.3x font scale, the same measured threshold `HealthAsk` uses) the name moves above its own lane, which costs the continuous finish line and keeps every word — the same trade the disclosure schedule makes. **The `race_results` half landed with plan 5 rather than here, and is recorded under this number because it is the deferred half of one decision, not a second decision.** `public.race_results(squad_id, local_date, standings jsonb, finalized_at)` snapshots the standing `rankRacers()` computed, written **once** by `finalize-days` when the **last** member of the squad finalizes that local date — days are per-user local (§2), so a result written on the first member's finalization would crown whoever's timezone happens to be furthest west, and `squadDayIsComplete()` therefore returns **false for an empty roster** rather than vacuously true, because `every` over an empty list is `true` and would permanently occupy a write-once primary key nothing would ever correct. Write-once is the §19 rule: a later Apple revision does not retract anyone's win, and it is why the standing is snapshotted at all — the projection behind it can no longer answer "who won on 14 March" once the buckets have been revised. **No client role holds any grant on the table.** A stored row is read by six different people, so it cannot carry deviation #47's per-viewer gate inside itself; `race_result(squad_id, local_date)` is a `security definer` read applying exactly the gate `squad_leaderboard()` applies — rank and species unconditionally, because a rank is not a health figure and species is already in two projections, and capped steps only when both sides have consented. The absent grant is a **stronger invariant than a policy and easier to check**: a policy can be subtly wrong, a missing grant cannot be subtly present, and a schema test pins the empty grant listing. A member who has not consented is stored with a real rank and a `capped_steps` of 0 rather than dropped, because dropping them would make the stored history disagree with the board their squad watched all day; the 0 is withheld again on the way out, so the substitution discloses nothing. A failed snapshot never stops a day becoming final — the day is the durable thing, and a squad with no history row for a date is what the digest already reads as "no result". |
...
| 51 | §5 and §6 name the stats **Agility**, **Strength**, Endurance, Vitality and Recovery, and deviation #41 reduced that set to Agility, Strength and Mind — the surface has spoken those words, and `boostChipLabel` has printed the raw three-letter keys, since the stat rail shipped | **The surface words become Body (`STR`), Motion (`AGI`) and Mind (`MND`).** Engine keys are unchanged everywhere — `CoreStat` is still `'AGI' | 'STR' | 'MND'`, the `daily_scores` columns are still `agi_points`/`str_points`/`mind_points`, the `profiles` rollups are still `agi_total`/`str_total`/`mnd_total`, `tiers` is still keyed `AGI`/`AGI_base`, and `program_weighted_total` still takes its arguments in that order. No migration, no change to `@kairo/core` at all. `STAT_NAMES` moves out of `src/ui/StatIcon.tsx` into a new zero-runtime-import `src/ui/stat-names.ts`, which also gains `dominanceName()` | Founder decision 2026-08-25, spec `docs/superpowers/specs/2026-08-25-body-motion-mind-design.md`, parent `…/2026-08-25-character-race-pivot-design.md` §5.5. Plan 2 of 5. **This is deviation #23's move in a second place** — that one made Bronze/Silver/Gold internal to scoring without touching a line of the engine, and this one does the same to the stat names, for the same reason: three-letter RPG abbreviations belong to a character sheet, and after the pivot the app asks somebody to race rather than to read one. "Body" is a word about the person; "STR" is a word about the system measuring them. **Mind does not move** — `MND` was already spoken as "Mind", so two words change, not three, and writing the change up as three invites an edit to the one line that is already correct. Three build facts are worth keeping. **The table had to move before it could be renamed.** `STAT_NAMES` lived in `StatIcon.tsx`, which imports `@expo/vector-icons` and therefore reaches React Native's Flow syntax that root Vitest cannot parse — so the stat words were **untestable**, and a rename touching seven call sites was about to happen with nothing able to fail. The new module imports one *type* and nothing else, the same split `read-types.ts`/`disclosure.ts` and `buffer.ts`/`milestone-store.ts` already use; `StatIcon.tsx` re-exports both names so not one call site changed. The rule that follows: **a module root Vitest tests imports `src/ui/stat-names.ts` by path, never `@/ui/index.ts`** — the barrel re-exports every component, and the alias does not resolve there either, which is why `program-copy.ts` reaches it relatively exactly as `goal-copy.ts` reaches `kairo-core`. **Two parallel copies of the words existed, and finding them is why the "one table" claim needed checking rather than asserting.** `STAT_NAMES` was the single source of *composition* and not of *content*: `DOMINANCE_LABELS` in `app/(tabs)/index.tsx` held its own `Record<CoreStat | 'balanced', string>` reading `'Agility build'`/`'Strength build'`, and is now `dominanceName()`, which covers the whole `Dominance` type including the null case so a parallel table cannot exist to drift; and `boostChipLabel` rendered `AGI ×1.5` on the squad board header — **the last surface in the app showing an engine name to a player**, and the one a rename would most obviously have missed, because it never contained a stat *word* to grep for. **Squad program names and Challenge area names deliberately did not move.** `squads.program` keeps its `strength` value labelled "Strength" and `ChallengeArea` keeps its: those name the *game* a squad or a session is playing, members consented to a squad under that name, and renaming them alongside the stats would be a second, unrequested change to a shared concept. What *is* wrong after the rename is a blurb, because a blurb names the stat a program weights — so each one now leads with the weighted stat's new word ("Body counts for more — effort and active calories"). The accepted cost is that a member of a Strength squad reads "Body counts for more", which is the correct sentence and briefly confusing; worth watching on the first device pass with a real Strength squad. **Only "Agility" is guardable, and the guard is a test rather than a convention** — `stat-names.test.ts` scans every non-test `.ts`/`.tsx` under `src` and `app` and expects no match. "Strength" is deliberately not guarded: it legitimately survives in five places (the squad program label, the Challenge area label, `STRENGTH_ACCURACY_NOTE`, and the `functionalStrengthTraining`/`traditionalStrengthTraining` `HKWorkoutActivityType` identifiers), and a noisy guard gets deleted. That scan caught two stale doc comments the plan had not predicted — `stat-detail.ts` and `daily-walk.ts` both quoted rendered copy back to the reader — which is the argument for the guard in one line. |
| 48 | §8: a squad goal is N-of-M — every member must hit the target individually, against a roster frozen at creation, with `required_members` as the denominator | **An Event is pooled.** `required_days` and `required_members` are dropped with their biconditional constraints; `event_progress()` returns a `pooled_value` window function over the date; `evaluateEvent()` sums one bar. When that bar is met, **every participant on the frozen roster is paid a completion row — including one who contributed nothing** | Founder decision 2026-08-25, same spec, §4.2. **This reversal is the entire reason the mechanic exists.** Under N-of-M a weak member was a liability and inviting somebody was a risk: their bad week cost everyone the goal. Pooled, the strong member carries — and *being carried* is a reason to be in a squad at all, which is what the solo-first product had no answer for. **Paying a non-contributor is the mechanic, not an oversight.** Paying only contributors rebuilds the per-member rule one layer down: the weak member would again be visibly carrying a liability tag, this time in the completion list rather than in the target. `planEventCompletions` therefore iterates the roster, not the contributors, and pays every member the same XP — the reward is a property of the commitment the squad made, not of who did the work inside it. **Three arithmetic traps follow, and all three are silent.** `pooledDays()` must **take each date once**: `event_progress()` repeats the pooled figure on every participant's row, so summing it naively multiplies every day by the squad size and reports a six-person squad as six times fitter than it is. `daysUnresolved` counts distinct **dates**, not rows, or a six-person squad's first day reads as six finalized days and the window is declared spent on day one. And a pooled date is **final only when every participant's day is** — a squad spans timezones, so one member's day finalizes hours before another's, and taking the first row's status would let a still-revisable contribution pay XP, defeating the final-days-only rule completion latching depends on. All three live in one function in `@kairo/core` for that reason: the client's bar and `finalize-days`' grading read the same code, so they cannot disagree about which day counted. **Grading reads the ungated column, and this was a real defect caught in build.** The plan had `finalize-days` grade off each participant's own `value`. That column sits behind #47's reciprocal consent gate, and the gate keys off the **viewer's profile row**, not their role — so a service-role run for a candidate who has not consented sees `NULL` on every row, pools the whole fight to zero, and silently never completes anything, with nothing anywhere to log. Grading reads `pooled_value`, which is never withheld. The per-member breakdown stays gated, and the detail screen prints "not sharing daily totals" rather than `0 kcal` for a withheld figure — null and zero are different facts, and the second accuses somebody of a quiet week they may not have had. **The pooled total is ungated on purpose, and the limit is recorded rather than pretended away:** joining an Event is itself an act of participation, and you cannot fight together without knowing how the fight is going — but **a two-person squad can invert the pooled total** by subtracting their own contribution. There is no version of a pooled mechanic that avoids this. It is named in the privacy policy alongside the rest. **Known risk, accepted:** pooling can hide a disengaged member indefinitely. That is the mechanic working, and it is also how a squad stops noticing somebody has stopped playing. The race (#46) is the counterweight — per-person and daily — which is part of why Battle ships after it rather than instead of it. |
| 49 | Nothing in the spec sets an Event target; §8 fixes a Goal's target at creation, and deviation #32 derives a Challenge's target fresh on every read from a trailing median | **An Event's target is snapshotted at creation and is a constant thereafter, while a Challenge's is derived on every read.** `bossHp()` in `@kairo/core` computes it once on the client; `create_event()` writes `p_target` **verbatim**. `EVENT_DIFFICULTIES` is `skirmish 0.6`, `standard 0.85`, `raid 1.15`, and `BOSS_HP_FLOOR_PER_MEMBER_DAY` is 150 kcal. Progress against it stays a read-time projection | Founder decision 2026-08-25, same spec, §4.3 and §4.4. **"Why is this one derived and that one stored" is the question the next reader will have, so both modules carry the answer in a comment.** A Challenge derives fresh because nothing stateful exists for a retroactive Apple revision to invalidate, and the trailing median moving **is** the mechanic — you clear one and the next is harder. An Event is the opposite case and inherits §8's Goal invariant: a target that moves mid-window silently re-grades every day already counted, which is precisely why a Challenge had to be a sibling concept rather than a `GoalKind` in the first place. **A boss whose HP rises because the squad got fitter mid-fight is that bug wearing a hat.** The target is fixed; the progress is replayed. **The client computes the number and the server stores it unchecked — the one place in Kairo that is true**, and a deliberate exception to §12. Reimplementing the trailing median in plpgsql would be a second implementation of the arithmetic needing a differential test, which is exactly what deviation #18 declined to pay for goal progress and declines again here. It is safe enough because the exposure is bounded to *a squad setting an easy boss for itself*, which costs that squad its own XP and affects nobody else, and because `eventCompletionXp` is capped at 500 and scales by the **window**, not by the target — so there is no way to convert an easy boss into disproportionate XP. **`standard` is deliberately under 1.** A cooperative mechanic that most squads lose is one most squads stop using, and a first Battle is where a squad learns that pooling works at all — so the default is winnable by carrying on as they already were, and `raid` is where pushing is required. The multipliers are **not printed on the creation screen**: they are the engine's, and 0.85 on a screen invites a squad to reason about the formula instead of about the fight. **The floor exists because zero is a real answer.** A brand-new squad's pooled median is 0, so without `BOSS_HP_FLOOR_PER_MEMBER_DAY` the boss has 0 HP and is defeated in the same second it is created — which reads as the feature being broken rather than as a gift. HP rounds to the nearest hundred, because the number is printed on a card and read aloud and "4,317 HP" claims a precision a trailing median does not have. **Submit stays disabled until the history query resolves**, and `bossHp` is passed `undefined` rather than a 0 fallback while it is in flight: a fallback would apply the floor and set a boss far easier than the squad's own history warrants, permanently, with nothing on screen to notice — deviation #37's fourth lesson in a new place. The history is fourteen sequential `squad_leaderboard()` calls, which is the honest cost of not adding a second aggregating function to review; it runs on the creation screen and nowhere else, and if it feels slow on device the fix is one `event_kcal_history()` RPC with the same consent gate, not caching. **One copy change departs from the plan, and the reason is this row.** The plan specified `challenge-copy.ts`'s sentence verbatim — *"Set from your squad's last two weeks, so it moves as you do"* — which is true of a Challenge and false beside a snapshotted number: it promises the one thing this deviation forbids. The screen says *"Set from your squad's last two weeks. Fixed once you start — the next one moves as you do"*, which keeps the legibility the borrowed sentence was for without the lie. **Adventure is written into the schema and nowhere else.** Both `kind` values and both `metric` values ship now so the migration happens once rather than twice, but nothing can create an Adventure — and a kind nothing can create is a kind nothing tests, so the first one will find whatever `distance_m` assumptions went unexercised. |
| 50 | §5.3 of the parent spec puts the daily loop on the character screen, gives quests one paragraph — "three a day, reset at midnight, auto-assigned from the account's trailing scored days" — and §4.4 names `StatRail` and the Strain/Sleep rows as what the disclosure gate keeps. Nothing in the spec adds a tab, and `TabPill` has been a three-disc orbit nav since the app shipped | **A fourth tab, `Today`, and three derived quests a day, built outside the disclosure gate.** `app/(tabs)/today.tsx` takes the Daily Walk and the Challenge door off the character screen's shelf and leads with a race summary card; the character screen keeps its hero, `TodayPanel`, `SyncStatus` and the disclosure note. `TabPill` grows a fourth disc: orbits 60→52, centre 74→68, bar gap `space.lg`→`space.md`, which is `3 × 52 + 68 + 3 × 16 = 272` against 320pt on the narrowest supported screen. `NAV_HEIGHT` stays 96, so `TAB_PILL_CLEARANCE` is unchanged — the discs got smaller, not the bar. Order is `['squad', 'index', 'today', 'profile']`, so Squad stays leftmost and You stays rightmost and **no existing thumb target moves to the other end of the bar**. **The character keeps the raised disc and stops being geometrically centred**: the raised disc means *anchor*, not *middle*, and with four items a raised third-of-four would be arbitrary while two raised discs is no anchor at all. Quests are **derived, never stored** — `pickQuests()` in `@kairo/core` is a pure hash of `(userId, localDate, tier)`, so the local-midnight reset costs no job, no row and no cron, and nothing stateful exists for a retroactive Apple revision to invalidate. That is the property `challenge.ts` established, and it is inherited from the same place: progress is a read-time projection over `health_buckets` and `daily_sleep`. Only the completion is stored, in `quest_completions`, written by `finalize-days` from **final** days with `on conflict do nothing`. A `quest_id` is opaque `text`, so a new quest costs no migration — and a **renamed** id orphans every completion banked against it, which is why the column comment forbids reuse. Migration `20260829090000_quests.sql`, shipped with `finalize-days` | Founder decision 2026-08-25, spec `docs/superpowers/specs/2026-08-25-the-today-tab-design.md`, parent `…/2026-08-25-character-race-pivot-design.md`. **Five things this row exists to record.** **(1) The tier rule measures engagement, not capability, and that is the spec's choice made explicit rather than silently improved.** `questTier()` keys off `trailingScoredDays`, a count of days that scored — so a thirty-day account averaging 3,000 steps is assigned the same tier as one averaging 15,000. The alternative, a trailing **median of daily steps** and the pattern `challenge.ts` uses, was considered and **not taken**: it makes the quest bar rise as the user improves, which is the exact conflation the Daily Walk exists to refuse and which §5.3 restates one paragraph later. The Profile override is therefore the correction for a rule that is wrong by construction for part of the cohort, which is why it ships in Phase 1 rather than being deferred, and why it **wins outright** — a rule that could veto it would make it a hint. The Profile copy names the automatic rule's actual input, so a user who finds their quests too easy understands why rather than assuming the app measured them and got it wrong. **(2) The client's tier read and `quest-plan.ts`'s must agree.** Both call the same `questTier()` with the same lifetime scored-day count (`total > 0`, on both sides) and the same `profiles.quest_tier_override`, with the override precedence inside that function rather than at either call site. A disagreement means the server grades and pays for three quests that were never on screen, and **the completion latches** — the sharpest failure in the subsystem, and a silent one. The sleep read is the same story in miniature: `finalize-days` reads through `scoringSleepMinutes` and the client through `scoredSleepMinutes`, so a hand-typed night — which scores no MND at all — reads "No reading yet" on the card and clears nothing on the server. A raw `daily_sleep.minutes` read on either side would pay XP for a bar the card never showed met. **(3) `recalculate_user_xp` gained a fourth source, and it is a full recompute written out whole.** Plans 3 and 4 both rewrite that function, so whichever lands second must read the deployed body first and carry the other's sources forward; landing blind silently drops a whole XP source and every affected account's level falls on the next write. Plan 4 landed first here, so `20260829090000_quests.sql` carries `event_completions`, `challenge_completions` **and** the three stat rollups forward verbatim and adds only `quest_completions`. The check is one query: `select prosrc from pg_proc where proname = 'recalculate_user_xp'`, and the schema suite asserts the body still names every source. Quest XP is never written into `daily_scores.xp_awarded` — a rescore replays that column from tier points and would wipe it — and never into `agi_total`/`str_total`/`mnd_total`, because a cleared quest is not activity in a stat. **(4) The migration is numbered *after* the Events one, not before.** Both rewrite the same function and migrations apply in filename order, so a quests migration sorting first would have had its whole XP change silently overwritten on every fresh apply — and would have passed every test in this repo while doing it, because the deployed database would have been correct. **(5) The disclosure gate is unchanged, including its subject list, and the parent spec reads the other way.** §4.4 names `StatRail` and the Strain/Sleep rows as what the gate keeps; read strictly that list is exhaustive, which takes Challenges out of the gate too. It is read as **illustrative**, and only quests leave. Three reasons, the first decisive: §5.3 of the same document says `/train`'s Challenges keep their name *and their behaviour*, unchanged — the redirect is a rule about who reaches the screen, so removing it contradicts a sentence the parent wrote about Challenges specifically, in service of a list it wrote about something else. §4.4's stated motive — a Today tab that "shows only the Daily Walk for three days" — is fully served without it, since the race card, three quests and the walk are all ungated and a day-one account meets three live things there. And a Challenge is not what teaches the loop: it is a trailing-median target derived from workout sessions the account may have none of, opt-in and off by default. So `StatRail`, the Strain/Sleep rows, `TrainEntry` and `/train`'s `resolved && stage === 'core'` redirect all stay exactly as they are; the constant, the threshold test, `useScoredDayCount`'s `total > 0` filter, the `resolved && stage` navigation rule and the retention measurement are untouched. The list is now written down in `useDisclosure`'s doc comment, because it never was — and that absence is what let the wider reading look plausible. |
| 52 | §14 schedules **three pushes a day** on a clock — 23:00 "1 hour left", 00:00 "Day ends", and a mid-morning "Day starts" conditional on the app not having been opened — inside a max of 3/day | **One digest per user per local day, at 08:00 in their own timezone, capped server-side.** `DISPATCH_HOURS`' three triggers collapse to `DIGEST_HOUR = 8` and `planDigest()`; the digest carries yesterday's **finished** race result, today's live standing and a live Event's pooled progress, and deep-links to `/today` | Founder decision 2026-08-25, spec `docs/superpowers/specs/2026-08-25-digest-and-level-response-design.md`. **08:00 and not the finalization moment**, which is the decision the shape rests on: days finalize roughly two hours after each user's local midnight, so a digest carrying the finalized result would fire at about 2am. The two are decoupled — `finalize-days` writes the result when the day closes, and the dispatcher sends it when the recipient is awake. The cron still fires hourly and twenty-three of those hours produce nothing, which is the normal path: every hour of the day is somebody's 08:00. **The cap lives in two places and both are load-bearing.** The exclusion inside `users_needing_digest()` is the *behaviour* — the ordinary path never attempts a second send, and there is no window between deciding and checking because the exclusion is a join in the same query that does the timezone arithmetic. `notification_log_one_digest_per_day`, a partial unique index, is the *guarantee* — a second insert fails even when the selection query is wrong. A client-side cap is not a cap at all: it is a race between the same account's phone and tablet. The index is **partial** so every other `kind` stays free to repeat, because those are bounded by `MAX_NOTIFICATIONS_PER_DAY` and moving that rule into the database would take it out of the module that owns and tests it. **`MAX_NOTIFICATIONS_PER_DAY` stays 3**: #52 caps the *scheduled* push at one and does not touch the budget bounding the event-driven ones — a digest plus an Event completion plus a Challenge clear is still three, and each of the latter two fires from something the user did. The three retired triggers **stay in `NotificationTrigger`** because `notification_log.kind` is free text with no check constraint, rows already say them, and a push sent minutes before the deploy can be tapped minutes after it — a historical value matching no case is a tap that goes nowhere, which is indistinguishable from push being broken. `daily_digest` is deliberately **not** quiet-hours exempt: the exemptions the retired evening pair needed were for 23:00 and 00:00, and 08:00 is never in quiet hours, so needing one would mean the schedule itself was wrong and the exemption would hide it. `users_at_local_hour()` was deliberately **not** dropped — dropping a general helper because one caller stopped using it is how the next feature reimplements it. Two open risks are recorded in the spec: one push a day may be too few to matter and there is no way to know before shipping it (the prior three were never measured against a control either), and 08:00 is the right *shape* but an unvalidated hour — moving it is a one-constant change plus a redeploy. **The client's copy lagged this by ten days and was corrected on 2026-09-04.** The permission sheet went on offering "when a new day starts and when this one is about to close" under a cap of three a day "except the two that close out your day, which arrive at 11 PM and midnight", and the Settings row still called the remaining push a "day-end reminder" — the retirement moved the server and left every sentence describing it in place, on the screen where a permission iOS grants once is spent. The sheet's words are `ask-copy.ts` now, pure and pinned to `DIGEST_HOUR` by a test, so the next schedule change fails something. That pass also removed **"three a day at most"** from both surfaces: `MAX_NOTIFICATIONS_PER_DAY` is 3 and stays 3, but `BUDGET_EXEMPT` sends bypass the budget *without consuming it*, so a digest, a cleared challenge and a beaten Event are four — a ceiling the engine does not enforce, printed as though it did. `RETIRED_PUSH_PHRASES` is one list both surfaces are tested against, on the payload-ban scan's precedent that two guards enforcing one rule drift apart. §14 of the spec is marked superseded in place; it is what the false copy was written from. **And the replacement's own first draft was false in a new way, caught in review**: it promised "never overnight", which `QUIET_HOURS` appears to guarantee — but quiet hours live in `planNotifications` and `finalize-days` does not call it, so `event_completed` and `challenge_cleared` fire at finalization, about two hours after local midnight. The copy states that timing now. **The underlying defect is recorded and not fixed here**: `notifications.ts` argues those sends should wait for morning, and the send path does not implement it; routing `finalize-days` through `planNotifications` would *drop* rather than defer them, which is a product decision and an Edge Function deploy, not a copy change. |
...
| 55 | Four Philippine endemic species with a picker, chosen in onboarding (#40) | **One Kairo.** Every character is a Philippine eagle. The picker, `/species` and `(onboard)/character` are retired; onboarding is `/connect` → `/name`. | Four species meant the app had no character — nothing can be *about* your Kairo when it is one of four interchangeable skins, and the picker spent the highest-attention moment in onboarding on an identity declaration. **Display-only and therefore reversible**: `profiles.species` is not migrated, not dropped and never written differently, so every stored choice survives and reversing this is deleting one line of `displaySpecies`. Six render boundaries resolve through it — `CharacterFigure`, `Diorama`'s habitat, both figures in `RaceLane`, `LeaderboardRow` and the event share row — and each lost its `Avatar` fallback, since `displaySpecies(null)` is an eagle. `CharacterFigure`'s View primitives went with those branches; `Build` keeps only the two fields the ground shadow reads. |
| 56 | Six characters running parallel horizontal lanes at one shared flag (#46) | **One shared sky corridor** everybody flies, drawn as rotated segments along a Bézier from `packages/kairo-core/src/sky-path.ts`, on its own tab. | The mechanics did not change — same payload, same client-side re-rank by capped steps, same derived `RACE_FINISH_LINE`, same reciprocal consent gate. Six parallel bars showed a quantity each; one lane shows a *field*, which is what makes "who is directly ahead of me" answerable at a glance. Drawn without `react-native-svg` deliberately: that is a native module, so it moves the EAS fingerprint, spends one of fifteen monthly builds and withholds OTA until it lands, and the rest of this redesign ships over the air. `SoloBoard` lost its track in the same pass — the corridor races the same ghosts, and two pictures of one race on two tabs is how they start disagreeing. |
| 57 | A squad is a **squad** (#26); the daily board is a race (#46) | **Flock and Sky** in the player-facing vocabulary. | Deviation #23's split in a third place: the surface gets the player's words and the engine keeps its own. `squads`, `squad_members`, `squad_leaderboard()`, `squads.program`, `race.ts`, `RACE_FINISH_LINE` and `rankRacers` are all unchanged, and so are the analytics values `squad_data_consent_granted`, `race_seen` and `quest_cleared` — those have historical rows behind them and `kairo_retention()` is deliberately unchanged across pivots. `SquadDataConsentSheet`'s body copy still says "squad", deliberately: it is the text members consented under, and rewording it belongs with the privacy-policy update rather than with a vocabulary pass. |
| 58 | The **Sunlit** palette and its Caprasimo/Figtree pairing (#53); the flat four-tab bar with every label painted (#54); the horizontal sky corridor (#56); onboarding as `/connect` → `/name`, two screens (#22, #55) | **Playful.** Same token names, new values: orange for you, violet for your lane and Mind, teal for rest, pink for the streak, plus two new families — **gold** (earned) and **sky** (the flight). Fredoka and Nunito replace Caprasimo and Figtree. The tab bar is frosted glass and names only the tab you are on. The Sky corridor is re-cut **vertical** and scrolled. Onboarding is six beats: `/welcome` → `/one-sky` → `/connect` → `/difficulty` → `/privacy` → `/name`, with three welcome cards after it. Settings moves off the You tab to `/settings`. | Founder decision 2026-08-30, from `Kairo Playful.dc.html`. **The scoring engine is untouched and no schema changed** — this is deviation #53's move in a third place, the token *names* carrying roles rather than hues, which is what let ~90 call sites re-skin without being edited. Four things are worth recording because they are where the design and the codebase disagreed and the codebase won. **(a) Cream on a bright fill.** The design draws white labels on orange, coral and gold; those measure 2.65:1, 2.93:1 and 1.52:1 and render perfectly while being unreadable. Four call sites shipped that pairing in the first pass — the active tab pill, the board's day toggle, the streak chip and `CtaPill` — so the rule is now asserted for every fill in `contrast.test.ts` ("a bright fill takes ink"), including the *failures*, so a future palette that made one dark enough for cream is a change somebody notices. `coralEdge` is pinned as carrying neither, because it is a lip. **(b) The onboarding order.** The design puts quest difficulty and privacy *after* the name, which is exactly deviation #22's trap — the profile row commits on the name screen and anything asked after that INSERT flips `resolveRoute` to 'ready' under an unfinished screen. They ask before it instead; but `quest_tier_override` and `squad_data_consent_at` are in `profiles`' column-level UPDATE grant and not its INSERT grant, so there is nothing to write to until the row exists. The answers are held in `useOnboardingAnswers` and written by the name screen after the insert. Nothing is *asked* after it, the row still commits exactly once, and both grants are respected. **(c) The vertical corridor.** `sky-path.ts`'s viewBox went 402×520 → 393×1560 and two cubics became three. `x` is no longer monotonic — the flight weaves — so that assertion was replaced rather than quietly kept; `dy < 0` is the invariant now. `BAND` was `0.11` of the *height*, which was the narrow axis and is now the long one; left alone it drew a 158pt band down a 361pt screen. **(d) "Dress your Kairo" is not built.** `character-assets.ts` says it outright — the cosmetic PNGs are flattened full-character previews, not composable layers, so a four-slot tray that stacks a cap, goggles and a bandana has no assets behind it. Parked at the founder's instruction; it needs re-cut layered art or a redesign to "pick one look". The **"Did you know?" beat (2d)** is built as a *phase* of `/connect` rather than a route, covering the real `readStepsToday` between the grant and the step reveal. `hatching-window.ts` adds a floor so the sentence is read rather than flashed, and names that trade: on a phone where the read returns instantly the beat is paced rather than caused, and one constant removes it. The window opens when `connectHealth` **resolves** — iOS has the permission sheet up during it, so a beat started at tap would spend its minimum behind that sheet — and comes down at the later of "minimum served" and "read finished", never the earlier. `trivia.ts` states no effect size: every number is the app's own constant or the size of an action, and a test bans a bare `%`. Two deliberate copy corrections: the design says "barkada" twice (retired by #26) and promises squadmates never see raw numbers (untrue since #47), and both are rewritten rather than transcribed. **Ships entirely over the air** — verified, not asserted: the tree's fingerprint is `324fba3e`, byte-identical to build 121853b7's, so `npm run eas:update:production` reaches every installed build and no EAS build is spent. Fredoka and Nunito are copied into `assets/fonts/` and loaded through `useFonts` rather than added as npm dependencies, precisely so `package.json` stays untouched. |
...
| 60 | §5.4: "HealthKit's active calorie estimation (STR) requires height, weight, and age for accuracy. **Collect these in onboarding.**", with a persistent soft prompt — *"Add your height and weight in Settings for more accurate STR tracking"* — for anyone who skips | **Never asked in onboarding, and documented as inert.** `height_cm`, `weight_kg` and `birth_year` stay in Settings, optional and gating nothing; the soft prompt is replaced by a standing note that says what they are actually for. | The premise is true of HealthKit and false of Kairo. Apple computes `activeEnergyBurned` against the body profile held in the **Health app**, before Kairo ever sees a calorie; `profiles.height_cm` and `profiles.weight_kg` are a second copy Apple never reads, and **no scoring path reads either** — not `planDay`, not `statPointsFor`, not the interpolation — so editing them moves no score, no rank and no quest. A question that changes nothing does not earn a screen in a run whose whole argument is that every ask has a visible why, and a prompt promising "more accurate Body tracking" sold a benefit the app cannot deliver. **The narrower claim is the true one on paper, and today even that overstates it:** `birth_year` *is* read, by `maxHeartRateForAge()` in the keystone's `strain.ts`, and Strain is display-only and never touches `daily_scores` — so the honest general statement is that no *scoring* path reads any of the three. But `TodayPanel`, the only surface that ever rendered Strain, was unmounted by deviation #59, so as of 2026-09-04 there is no live reader of any of the three and the player-facing note names none. The ticket asked the note to name Strain; that instruction was written three days after the surface stopped existing, and a test now holds the note clear of it. Reversible and cheap: the columns, their CHECK constraints, the column-level UPDATE grant and the editor are all untouched, so collecting them later is adding a screen rather than restoring a schema. The card's copy lives in `BODY_METRICS_NOTE` beside the validation rather than in the `.tsx`, because root Vitest cannot load a component and a claim about scoring is exactly the sort that should fail a test when it stops being true. |
| 61 | §5: the notification ask waits for a squad or a running Battle, on the rule that every ask has a visible why | **A first scored day is a third why.** `shouldAskForNotifications` returns true on `hasSquad \|\| hasEvent \|\| hasScoredDay`; a player with no squad and nothing scored is still not asked. | Founder decision, onboarding curation spec 2026-09-04, built the same day. The two original reasons are both social, which was right while the pushes they enabled were social. Deviation #52 retired all three of those and left one scheduled push — the 08:00 digest — and **Kairo is solo-first**, so `hasSquad \|\| hasEvent` structurally excluded the entire solo cohort from the only re-engagement the app has. A scored day is the moment there is genuinely something to say at 8am tomorrow, which is the same test the other two reasons pass, applied to the solo loop. **This adds a reason, not a surface**, and that distinction is the whole risk: the primer sheet, the Health-first ordering in `permissions/ask-order.ts`, the one-ask-per-session latch and the single modal host are all untouched, because two `<Modal>`s presenting on one root view controller is the defect that ordering function exists to prevent — the ask-order suite gained cases pinning each of those guarantees *for the newly eligible cohort* rather than only in the abstract. `hasScoredDay` reads the Today tab's own `useScoredDayCount` key, so it costs no request and the two cannot disagree in one frame; it is lifetime and `total > 0` for that query's own reasons, since `sync-health` writes a row per date in the payload whether or not it scored and a bare row count reads 2 on install — which would put this ask back inside onboarding, the one place §5 forbids it. A count still in flight reads false, withholding the ask for a frame rather than presenting it on a guess. `notification_ask_answered` fires **per answer** — `granted`, `declined` or `deferred`, and nothing else in the payload — because "Not now" never reaches the system dialog and a deferral can genuinely recur; the widening is judged on grant rate, and the eligibility reason is a health fact that may not ride along. Two surfaces were saying the old policy and are corrected with it: the sheet's doc comment, and the Settings row's undetermined help line, which told solo players they needed a squad to be asked. |
| 62 | The onboarding run is six beats — `/welcome` → `/one-sky` → `/connect` → `/difficulty` → `/privacy` → `/name` (#58) | **Seven beats: `/mirror` sits third**, between the sky card and the Health ask. Both skip affordances land on it; it carries no skip of its own. | Founder decision, onboarding curation spec 2026-09-04, built the same day. The two cards above it say what Kairo is and what the game is, and both are pitches — nothing in the run addressed the person who believes their own days are not worth reading, which is exactly the person most likely to refuse the next screen. `/connect` presents the one dialog whose refusal cannot be undone from inside the app, so it gets the run's best framing directly above it: *you're not lazy, you're just not being counted.* **Skip moves with it.** `onboardingSkipTarget()` derives the destination as the last beat of the opening phase rather than naming a route twice — skip's purpose is getting past the pitch, and the pitch *is* phase 0 — because landing past the mirror beat would route the people most likely to decline around the argument written for them. **The rail did not change**: still four phases, still what this is / letting it in / your choices / the name, with only the fills and partials inside phase 0 moving, and every one of them derived by the registry deviation #58's run already reads from. The **paged dots** became honest without being edited — they promised three cards while two existed — and are deliberately not retired, since the rail answers how far through the run and the dots answer which value card, and they are already hidden from assistive technology so only one thing announces position. Kairo appears as a **pose with a heavy ground shadow** and the `tired` reaction still has **no producer**: sleepiness is a daily Mind state rather than an event, and an onboarding screen has no account state to key an occurrence against, so wiring one here would breach a stated invariant in passing. Deviation #22 is untouched — the beat is added *before* the name, the profile row still commits exactly once, and nothing is asked after the INSERT. Each beat now also records **one impression**, `onboarding_beat_seen` with `{ route }` and nothing else, emitted by a single hook that takes a beat name so no screen can reach the payload; unguarded, because the run happens once per account and a back-and-forward duplicate is absorbed by counting distinct beats. Before it, `onboarding_started` fired on the connect beat and `profile_created` at the end, so the cost of adding a beat was unmeasurable — which is the thing a curation pass has to be able to see. |
| 63 | §5.3 and deviation #50: a new account's quest tier comes from `questTier()`'s trailing scored days, and the Profile override is the correction for it. A trailing **median of daily steps** was considered in #50 and explicitly **not taken** | **Calibrated once, at the Health grant.** `/connect` reads fourteen complete local days of step totals off the phone, `calibrateQuestTier()` in the keystone takes their median and proposes the highest tier whose entry bar it clears, and `/difficulty` opens with the measurement stated above the choices it already renders, with the proposal pre-selected. The player's answer still wins outright, there and in Settings. **An onboarded account is therefore no longer on Automatic by default** — that is the intended change, and Automatic survives as the fallback. | Founder decision, onboarding curation spec 2026-09-04, built the same day. **Deviation #50 rejected the trailing median and this adopts it, and the two are not in conflict — the difference is that a rule re-reads and a seed does not.** #50's argument is that a bar which rises as the player improves conflates capability with effort, which is the exact thing the Daily Walk exists to refuse; that argument is about a *standing* rule, and it still holds, so `questTier()` is untouched and still the fallback. Read once and written into `quest_tier_override`, the same median cannot rise, because nothing re-reads it. `questTier`'s own comment now records both halves — without it the code says the median was refused while the app ships it. **What the auto rule was actually measuring is why this was worth doing:** on day one every account is a Starter, so an athlete is dealt bars they clear before breakfast, and a genuinely gentle walker is promoted a week later for reasons that have nothing to do with them — quest size was measuring how long somebody had been here. **Five rules, all in one pure function in `quest.ts`** rather than a module of its own, because it needs both the tier rule and the catalogue and a sibling importing both is an import cycle the moment either wants the result back — the alternative, threading `QUEST_CATALOGUE` through as an argument, is the mistake `TIER_POINTS` already made and broke an out-of-package caller at runtime rather than compile time. **(1) Today is excluded**: calibration runs seconds after a grant usually taken mid-morning, and a partial day in the set drags the median down by roughly half a band. **(2) Zero days are dropped, not counted** — a zero-sum day is indistinguishable from a phone in a drawer or a phone bought last week, so counting them would median a new-phone player to the floor while the screen claims to have measured them. **(3) Four qualifying days minimum**, below which the outcome is `no-history` and the tier falls back to Automatic; a fortnight of zeroes is `no-history`, never Starter. **(4) The median, never the mean**, so one long hike promotes nobody — `median()` is now one module shared with `challenge.ts` rather than restated. **(5) Bands are each tier's *minimum* steps target derived from the catalogue** — met on a good day, not already beaten on a median one — and pinned as literals by the same test, the `DAILY_STEP_BASELINE` arrangement, because moving a band silently re-sorts every new account. **The read is deliberately narrow.** `readDailySteps` is one daily-interval step collection, not `readHealthWindow` over fourteen days, which runs six hourly collections plus every workout sample plus sleep — including heart rate, owner-readable only and absent from every projection. Reading that much to propose a quest size would leave the beat's own privacy claim technically accurate and morally misleading; `calibration-read.test.ts` scans the function and fails if it widens. **Nothing about those fourteen days leaves the phone.** The median crosses from the connect beat to the difficulty beat in `useOnboardingAnswers`, which is already cleared on commit; it is never written to `profiles` and never enters a telemetry payload, and a scan holds both. `calibration_completed` carries `{ outcome }` and **not the tier proposed** — the question is whether calibration can read a new account at all, and a tier breakdown would be a distribution of the cohort's fitness sitting in `app_events` to answer a question nobody asked. Once ever, on an MMKV marker, because re-entering `/connect` and granting again re-runs the reading. **No calibration screen is built**: with the proposal pre-selected, a screen saying "we'd start you on Steady" followed by one asking "how big?" with Steady already chosen is two screens for one decision. Deviation #22 is untouched — nothing is asked after the INSERT and the profile row still commits exactly once, on the name beat. Settings' help line described the automatic rule as what applies to everybody and is corrected in the same commit. |
| 64 | Deviation #58: onboarding is followed by **three welcome cards on Today**, the third of them a flock nudge with a single invite CTA. The onboarding curation design proposed the flock ask as a **separately leased first-run sheet** on Today | **Four welcome cards, and the flock ask is the fourth.** Card three keeps the *reason* — a flock is up to `FREE_SQUAD_MAX_MEMBERS` birds chasing one flag — and becomes a linear read; card four is the ask, with three real options: **I have a code** · **Invite a friend** · **Not now**. Both doors land on the Flock tab, which owns joining and sharing already. | Founder decision, onboarding curation spec 2026-09-04, built the same day. Kairo is a race and a new account lands on Today with an empty sky and nothing prompting it to be filled, so the social loop deserves a first-day chance. **It cannot be an onboarding route**: joining or creating a squad needs the profile row, so the ask has to come after the INSERT, and a route after the INSERT is deviation #22's trap exactly. **And it must not be a second sheet.** The welcome cards are already a once-ever first-run modal on Today leasing the same root view controller (`modal-owner.ts`) — a separately leased flock sheet means a brand-new account meets both on the same first focus, one loses the lease, and the loser reappears later out of context. As a fourth card it needs **no new modal owner, no second once-ever marker and no ordering rule between two first-run surfaces**, and it lands in a sequence the player is already tapping through. The card list gains an optional actions slot that **only this entry uses**; a test asserts exactly one card carries it, because a second decision point in a run of linear reads is the thing this arrangement is trading against. **A known and accepted loss, recorded in the component rather than left to be rediscovered:** `welcome_seen` is claimed when the run *opens*, not when it finishes, so somebody who force-quits before card four never sees the ask. Two repairs suggest themselves and both are refused — moving the ask to the front is the ask arriving before its why, which is the rule the notification policy already enforces, and a second marker would fix it while reintroducing the two-surface ordering problem the arrangement exists to avoid. The loss is **bounded**, because the Sky tab's flock rail carries a permanent trailing invite slot, so a missed card costs a nudge rather than the feature. **The join door is withheld from somebody who already has a squad** — their own invite code is the proof of one and the free tier holds exactly one, so a join door for them is a path that can only fail; they are offered the share instead. `flock_prompt_answered` carries `{ answer }` — `joined`, `invited` or `skipped` — and **records which door was taken, not what came of it**: `squad_joined` and `squad_created` already say whether a squad resulted, and folding the two together would make the card look like it converts far better than it does. It needs no marker of its own, because the run's `welcome_seen` already makes the card unreachable twice. Every word of the run moved into `welcome-cards.ts`, zero-runtime-import apart from the keystone and `theme.ts`, so root Vitest can hold the copy honest — the same arrangement `kairo-voice.ts` and `ask-copy.ts` have, and the reason `RACE_FINISH_LINE` and `FREE_SQUAD_MAX_MEMBERS` are read from the constants rather than restated. The request crosses to the Flock tab as `?pane=join`, **consumed and cleared** on arrival, because Expo Router keeps tab screens mounted and a parameter left in place would reopen the form on every later visit; the href and the parser live in one module (`flock-pane.ts`) so a typo cannot make one side silently disagree with the other. **Three things the review caught and the codebase won on.** **(a) The sheet had to be bounded.** It was unbounded by design — "as tall as its copy" — which was survivable with one CTA and is not with two pills and a decline under a 240pt art panel: at the largest content sizes the child clipped off the bottom is "Not now", which is the permission sheet's 2026-08-17 failure exactly, on the one card that must be declinable. It now carries all three halves of that lesson — `maxHeight: '90%'`, a `ScrollView` that is `flexGrow: 0, flexShrink: 1`, and content in a View with a computed **point** width — and a test pins all three. The art scrolls with the rest rather than being pinned, because a pinned panel eats the bound. **(b) The label did not fit.** This pill sits in a sheet inside a scrim and has lost four lots of `space.lg`; at the `chrome` scale's 1.4× cap a three-word label no longer fits on a 320pt screen, and `OnboardingCta`'s `numberOfLines={1}` ellipsises it — a control whose words are cut is one somebody cannot act on. The component gained a `lines?: 1 | 2` prop, default 1 so no beat moved, and the design's "Paste an invite code" became **"I have a code"**, which says the same thing shorter and in the player's voice. **(c) Two actions could fire in one frame.** `setOpen(false)` lands on the next render and RN can deliver taps to two `Pressable`s in one frame, so a fast double-tap filed two rows *and* pushed both destinations; a ref guard bounds it, since `welcome_seen` bounds the card and not the frame. |
| 65 | The Digest reaches everybody who has a reason to be notified (§14, deviation #52) | **`users_needing_digest()` closes on lapsed solo players.** It gains an activity predicate and stops sending to an account with **no scored day in seven local days**, resuming by itself on the next one; the function is replaced in place, so `dispatch-notifications` needs no redeploy. Written and shipped alongside deviation #61's widening of the ask itself, which is what makes an account reachable in the first place. | **The two halves are one deviation split across two rows because shipping either alone is worse than shipping neither, and #61 already covers the ask.** Deviation #61 opened `shouldAskForNotifications` to a solo player's first scored day, but a digest with no suppression sends a lapsed solo player a push every morning they would never previously have received — thirty a month to somebody who left, which is how a channel gets switched off permanently. Applying the suppression without #61's ask changes nothing, since that population holds no token. **The window is exactly seven local days ending today** — six days ago qualifies, seven and eight do not — and the schema suite pins both sides, because this is the boundary a future edit is most likely to move by one. `total > 0` is the same reading of "scored" every other surface uses. **A *lapse* is not a *quiet week***: a player who is here and scoring little scores, so they pass the predicate every day; lapse stops the Digest and nothing else — no demotion, no lost Mastery, nothing stored, and the player is never told. The seven is a commented literal in SQL, not a mirrored TypeScript constant, because no client ever asks whether it is suppressed. **Two ordering constraints:** the migration ships before the client, or solo players hold tokens against an unsuppressed Digest; and the landing page deploys before the OTA that drops the invite message's privacy clause, or the claim briefly exists on no surface a non-user can reach. |
