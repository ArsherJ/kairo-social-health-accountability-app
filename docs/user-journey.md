...
- **The Sky has a minimap on its right edge** and a finer continuous-looking trail. The whole flight, every shareable bird, ridge and viewport window still derive from the existing `flightFrame`, race-progress and minimap math; navigation paint did not become a second progression rule. When a bird is too near the ground for its fixed label to clear the tab bar, the label moves above it without moving the bird's path anchor.
