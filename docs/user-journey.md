...
- **Four tabs — Today · Sky · Flock · You**, in a quiet rounded bar with a compact lilac selection, readable labels and no raised centre action. **The app follows the phone's appearance** through warm-pastel light and dark schemes: cream/charcoal pages, apricot primary action, lilac support and mint secondary surfaces under one set of token roles. Settings → Appearance can override System; `userInterfaceStyle: 'automatic'` is a native field, so it ships with a build.
- **Today combines progress and character**: date/name and Level/Streak lead into one responsive hero with Motion and its meter beside the resolved plush eagle. The next-step sentence/details action, Body and Mind, then three quest rows follow. Motion appears once. #59's figure/reaction rules and the quest contract remain; there is no score total, race copy or Mastery.
- **The Sky is open-air flight toward layered ridges**, with a scrubbable minimap on its right edge and no winding trail. Steps determine straight forward distance; automatic sideways drifting is decorative, not steering or additional progress. The birds, map and Locate use the same presentation coordinates, while rankings, finish line and measured safe clearances stay intact. Reduce Motion stops ambient movement. Ground-level labels can move above their birds to clear the tab bar.
- **Flock opens with a compact gathering before the board; You opens with one portrait/header before streaks, selectable records, growth and calendar.** A long perch name widens its own horizontally scrollable card and wraps rather than being cut to two lines at large text. Both tabs preserve their existing actions and truthful private/empty branches.
- **All seven onboarding views use the same warm-pastel theme in light and dark.** The Privacy policy link remains an explicit action on the privacy beat; it opens the public policy and changes no answer.
- *(Superseded 2026-09-10 by the dashboard bullet above; kept because the figure rules it states are still live.)* **Today was the Living Mirror** (deviation #59): the KAIRO scene, compact Level and personal Streak, one Motion figure, one quest-backed next step, and **See today's details**. The figure's **scale falls with the growth stage** (issue #33); per-stage artwork existed from 2026-09-08 (issue #31) until deviation #73 replaced the whole pack with one body on 2026-09-11. Its **crest takes the dominant stat's hue** from the same date, on this screen and on a flock row alike. It carries no race copy, no Mastery coins, no quest rings, no sleep or lane tiles, no Daily Walk card and no Challenge card.
- **The daily race lives on the Sky.** Crossing the line *is* clearing the Daily Walk: `RACE_FINISH_LINE` is `DAILY_STEP_BASELINE`. A player alone gets open sky, the ridge named as the opponent, and an invitation where the rivals would be — never a rank and never a fabricated rival (2026-09-02).
...
- **The privacy claim is made from a declared list, and one test owns all of it** (2026-09-07, issues #19 and #27). The surfaces are the privacy policy, the invite landing page, the HealthKit permission sheet — its derived type list **and** the fine print under it, which had been hand-written in the component and guarded by nothing until the sweep found it — the privacy beat, the Health ask on `/connect`, and the invite *message* — registered as a surface that makes **no** claim and points at one that does, because no honest version fit the room it had and the compressed one it carried was both stale and self-contradictory. `src/features/privacy/claim-surfaces.test.ts` is the one scan; it replaced three, which is how a fourth surface making the claim came to have nowhere to be registered. In-app copy lives in `claim-copy.ts` and no screen may write its own.
...
`welcome` → `one-sky` → `mirror` → `connect` → `difficulty` → `privacy` → `name`, all under `app/(onboard)/`. **Seven beats as of 2026-09-04**, six since 2026-08-30 (deviation #58), where it was two: three value cards open the run, the Health ask sits in the middle, and two choices are collected before the name. Every beat carries the same four-segment rail, so the run always says how much is left — a run of two had no shape worth drawing and a run of seven does. The spec's ordering principle, and the reason for it: iOS gives one clean shot at the HealthKit and notification prompts, so stacking every permission ask before any fun front-loads friction that costs signups.

**The run is declared once, in `src/features/onboarding/beats.ts` (2026-09-04).** Each beat used to hand-write its own rail position — `filled={2} partial={0.5}` and so on across seven render sites, arithmetic restated seven times against a shape nothing declared. The registry declares each beat's *step* instead and derives `filled` and `partial` from it, so adding a beat is one entry and the count re-derives itself. **The rail draws one segment per beat, "Step n of 7" (deviation #75, 2026-09-14)** — it measured four phases for seven beats so that adding a beat never lengthened the run, and testers read a half-filled segment as no progress and the run as stalled. The hatch is the one thing that shares a step: a wait the player cannot act on fills the Health ask's segment rather than opening its own. *Phase* survives as the `pitch` flag Skip and the value-card dots derive from. The registry also owns each beat's **button words**, which are now its own: *Let's fly*, *I'm in*, *Show me*, *Connect Apple Health*, *Lock it in*, *Good to know*, *Say hello*. Three identical "Next" taps told a new account nothing about where they were, on the one run whose whole job is orientation. `beat-registry.test.ts` scans both directories and fails any screen that puts a literal rail position or a hardcoded CTA label back, that hand-writes where Skip lands, or that reports its impression itself.

**The mirror beat is third, and it exists to move the blame before the ask** (`mirror.tsx`, 2026-09-04). *You're not lazy — you're just not being counted.* Most days disappear the moment they end; nothing saw the walk to the jeepney stop, the stairs, the long way home; Kairo counts them, and that turns out to be enough. Its button is soft: *Show me*. The welcome card says what Kairo is and the sky card says what the game is, and both are pitches — nothing above this beat addressed the person who believes their own days are not worth reading, which is exactly the person most likely to refuse the next screen. The 2026-09-11 presentation supersedes its fixed dusk-only treatment: the same pose and ground emphasis now belong to the coherent light/dark onboarding theme. It gives the `tired` reaction **no producer**: that reaction is declared without one deliberately, because sleepiness is a daily Mind state rather than an event and an onboarding screen has no account state to key an occurrence against.

**Both skip affordances land on the mirror beat, and it carries no skip of its own.** `onboardingSkipTarget()` derives the destination as the last beat of the opening phase rather than naming a route twice — skip's purpose is getting past the pitch, and the pitch *is* phase 0. Both cards used to name `/connect`, which was right while the pitch ended there; landing past the mirror beat would route the people most likely to decline around the argument written for them. Once you are on the last beat of the pitch there is nothing left to skip.

**The paged dots became honest without being edited.** They promised three cards while two existed; the mirror beat is the third. They are not retired and do not duplicate the rail — the rail answers how far through the run, the dots answer which value card, and the dots are already hidden from assistive technology so only one thing announces position.

**Each beat records one impression**, `onboarding_beat_seen` with `{ route }` and nothing else, emitted by `useBeatImpression` on mount. One emitter taking a beat name is what makes "the route name only" true by construction: no screen can reach the payload, and `/connect` is holding today's step count while it reports. It is unguarded — the run happens once per account, so the funnel is honest without a marker store and a back-and-forward duplicate is absorbed by counting distinct beats. The hatch reports nothing; it is a phase of `/connect`, whose own impression already covers the moment. Before this, `onboarding_started` fired on the connect beat and `profile_created` at the end, so every beat between them was invisible and the cost of adding one was unmeasurable.

**Health moved from fourth to first on 2026-08-17**, to *third* on 2026-08-30, and to *fourth again* on 2026-09-04 — behind the mirror beat, which is a framing rather than another pitch. Asking for it after sign-in, a character choice and a name meant the first screen a new user landed on was a dashboard of zeroes — the app at its least convincing, at the moment it had just spent all its credit. But asking for it *first* meant the very first thing a brand-new account saw was a permission request, before anything had said what it was for — the worst possible order for the one dialog whose refusal cannot be undone from inside the app. Value cards now come first and nothing else moved: Health is still asked before the name, so the home tab still lands on real numbers. `redirectTarget`'s `needs-profile` case returns `/welcome`.
...
1. **Connect Apple Health** (`connect.tsx`). Explains what Kairo reads and states the privacy claim in the privacy beat's own words — *daily totals only: steps, distance, active calories and sleep; never your route, never an hour-by-hour trail, and only where you have both agreed* — then asks. **That line was stale until 2026-09-07** (issue #27): it promised the squad sees your progress "never the raw numbers", false since deviation #47's per-row consent gate, and named "active minutes", not a stat since deviation #41 — on the screen a 5.1.3 reviewer reads, two beats before the privacy beat wording the same claim correctly. It deliberately does **not** enumerate the eight read types: Apple's own sheet is the authority on that list, and naming four of them here as though they were all of them would understate the ask. After the sheet resolves it **reads HealthKit locally and shows today's real step count** — `readStepsToday` in `src/features/health/read.ts`, keyed to the *device* timezone because `profiles.timezone` does not exist yet. This works this early precisely because it needs no server: there is no profile row, so nothing for `health_buckets` to hang from, and the first `sync-health` call still happens after `/name`. The reveal is set exactly as the home screen's hero is — same `Numeral size="hero"`, same `accent[700]`, same display-face unit — so the home tab reads as a promise kept rather than a different screen. Zero steps and a read that throws are treated identically and neither is an error: *"We'll pick up your activity as it comes in."* A connect that actually **failed** is a different case and says so, rather than advancing to the reveal — a failure that reached that copy would be indistinguishable from a quiet phone, and the user would go on with a character powered by nothing. **"Not now" is a deferral, not a refusal** — `PermissionAsks` asks again later. This screen fires `onboarding_started`, which names the start of onboarding rather than any particular screen, plus `health_ask_completed` or `health_permission_failed`.

**The grant also calibrates the starting quest size** (deviation #63, 2026-09-04). Alongside today's total, `connect()` runs `runCalibration` — one narrow HealthKit read of the **fourteen complete local days ending yesterday**, step counts only, deliberately not the sync path's window read. `calibrateQuestTier()` in the keystone drops zero days, requires four qualifying ones, takes the **median** and proposes the highest tier whose entry bar it clears; below four days the outcome is `no-history` and the tier stays Automatic. Today is excluded because the grant is usually taken mid-morning and a partial day drags the median down by roughly half a band. The hatch waits for **both** reads, so `/difficulty` never mounts with no proposal and then re-renders with one under the player's thumb. The proposal lands in `useOnboardingAnswers` and is pre-selected on the difficulty beat; nothing about those fourteen days is stored or sent, and `calibration_completed` reports `{ outcome }` once ever and never which tier was proposed. **There is no calibration screen** — with the proposal already selected, "we'd start you on Steady" followed by "how big?" with Steady ticked is two screens for one decision, so `/difficulty` simply states the reading above the choices it already renders. It is a **seed, not a rule**: read once at the start and never re-derived, which is what makes the same trailing median that deviation #50 refuses as a standing rule safe here. So **an onboarded account is no longer on Automatic by default**, and Automatic is the fallback — for accounts that predate calibration, hit `no-history`, skipped the beat, or clear their override in Settings. The player's own answer wins outright over both: `questTierChosen` is what tells a seeded tier from a chosen one, so re-entering `/connect` and granting again cannot reach forward and undo a choice already made.
...
### One push a day, in the morning (deviations #52, #61, #65)
...
has left is thirty a month, and what people do about that is switch
notifications off permanently — which costs the channel for the moment they
might have come back. **The two halves ship together**, because opening the ask
without the suppression gives a lapsed solo player a daily push they would never
previously have received, which is worse than the bug being fixed.

Three things about lapse are deliberate and easy to misread. An account that has
**never** scored receives nothing, which looks like a bug that eats somebody's
first digest and is not one — the ask now fires on the first scored day, so such
an account holds no push token; the two rules meet at the same boundary from
opposite sides, and a young-account exemption would be a second rule to
disagree with the first. Nothing about the account changes: no demotion, no lost
Mastery, no altered gate, no column, no telling the player. And **a lapse is not
a quiet week** — a player who is here and scoring little scores, so they pass
...
Two rules, one boolean, and the accused hears it first.

An hour that could not have happened to a person **flags the day**: more than 12,000 steps, 15,000 m or 1,200 active kcal in one hour (exercise minutes have been capped at 60 since the column was written). Beside it sits the older rule — a step burst nothing corroborates, no workout, no heart rate, no distance consistent with the stride. `daily_scores.flagged` stays a **boolean** because both produce the same sentence for a player; a reason column would store a distinction nothing draws.
...
**Distance is not GPS**, and correcting that comment changed no rule. On iPhone `DistanceWalkingRunning` is estimated by the motion coprocessor from the same pedometer stream the steps come from. Requiring a workout or a heart rate *beside* the distance was proposed and rejected: it would make `MIN_PLAUSIBLE_STRIDE_M` dead code, since a workout returns early and heart rate alone already clears — and it would flag the honest phone-only runner, whose hour at running cadence clears the 9,000-step bar with nothing but the distance to vouch for them. The attack it aimed at is killed at the source instead, because a fabricated distance sample is typed in too.
...
`app/(tabs)/_layout.tsx` defines the shell every session lands in after onboarding. Since 2026-08-27 (deviation #54, superseding #50) there are four: **Today · Sky · Flock · You**. The current shared `TabBar` is a quiet rounded surface: every item keeps a readable label, and the selected one gets a compact lilac wash rather than a raised centre action or four competing gradients. The label and selected accessibility state travel with the same control. `NAV_HEIGHT` stays 96, so `TAB_PILL_CLEARANCE` did not move and no screen's bottom padding changed. The earlier expanding, selected-label-only bar is historical presentation, not a second current option.
...
> **Superseded again 2026-09-11 by the warm-pastel composition.** Today is a dashboard whose first hero combines the Motion reading/meter with the resolved plush eagle; the sentence/details action, Body and Mind, then three quest rows follow. Motion appears once. Every rule below about the figure, quests, reaction and details sheet still holds; `today-board.ts` remains the reading-copy authority.

Today was the **Living Mirror**: KAIRO remains the largest visual, standing in a Motion location derived from live steps against `DAILY_STEP_BASELINE`. Compact Level and personal Streak remain in the scene; the day has one large raw reading (steps), one gentle next step selected from the unchanged three daily quests, and **See today's details**. Details contains Motion steps/distance/Daily Walk run, Body active energy and verified strength minutes when present, verified Mind sleep only when capable and measured, every quest state, relevant sync help, progress help, and the `full`-gated Challenge link. Each of the two shifts explains itself there as one sentence when it fired: `spreadLine` last in the Motion section, and — since 2026-09-08 (deviation #68) — `restedLine` last in the Body section, *"Slept 8 hours — Body tops out 50 kcal sooner today."* Both report the discount rather than the moved figure, and the second reaches only accounts with a sleep source. The Sky tab owns the race, You owns Mastery and records, and opening Kairo is never required for activity to count.
...
- **Three stats drive it, as of 2026-08-20** (deviation #41, which supersedes §5's and §6's four-stat tables): **Motion** (steps), **Body** (active calories) and **Mind** (sleep). Those are the words on every screen; `AGI`, `STR` and `MND` are the engine keys underneath them and are rendered nowhere, as of deviation #51. Two of them are phone-only and passive; MND needs a sleep source, typically a wearable. END (active minutes) and VIT (hourly movement) are not gone so much as demoted from stats to *modifiers*: active hours lower Motion's bands by 5% a step to a cap of 25%, so the signal survives as generosity rather than as points. **A modifier is a threshold shift and never a multiplier**, because a stored multiplier would stack with the squad program's read-time ×1.5 (deviation #10's trap). Two later corrections to that sentence: verified workout minutes stopped lowering Body's bands on 2026-08-29 and raise Body's *raw value* instead, and since 2026-09-08 (deviation #68) the thing that lowers Body's bands is **last night** — a rested night, up to 12.5%, tapering past nine hours exactly as the Mind curve does. Motion's shift is untouched by all of it.
...
- **"How Kairo counts your activity"** (`app/counting.tsx`, 2026-09-14, issue #45), reached from Settings above the privacy policy, from the privacy beat beside the policy link (inert in the account-free preview; `redirectTarget()` lets a profile-less account stand on `/counting`, the one route outside `(onboard)` it may, because the simulator walkthrough found the gate bouncing the reader back to `/welcome` the instant it was pushed), and from the Today details sheet beside "How progress works" — one tap from "Steps from X aren't counted yet" or a flag, which is when a player wants it. Five sections, in the glossary's words: which sources are **counted** (the phone, the watch, other Apple-recorded devices; another app's steps not yet, without accusation), what is **typed in** and left out, where the day stops (the **ridge**, named through `RACE_FINISH_LINE`, and each stat's own top), what a **flag** means (no points lost; no personal best that day; the flock sees it; that day only, tomorrow starts clean), and where it happens (sorting on the phone, the server re-scoring each day from stored hours). It never names the per-hour bar — `counting-copy.test.ts` bans the ceiling figures, formatted and bare. Not an eighth beat, and not on the Health ask, which is the disclosure surface the privacy sweep reads whole.
...
> **Superseded 2026-09-01 (deviation #59), and kept because the reasoning below is still why each of these exists.** This describes Today between deviations #54 and #59. There is no "below the fold" on Today any more: the quest cards, the Daily Walk card and the Challenge card are gone from the screen, replaced by one quest-backed next step and a details sheet. The quest *mechanics* below are unchanged and still exactly right; only their presentation moved. **This is the contradiction the ground-truth block exists to make visible** — the section above already says the Daily Walk card was removed, and this heading has claimed since 2026-09-01 that it is one of three things under the hero.
...
> **Open-flight refinement, 2026-09-11:** the visible curved trail has been removed from the scene and minimap. Birds advance toward the ridge in proportion to earned steps and drift automatically sideways, with no steering control and no time-based advancement. Soft ridge layers and clouds make the default view a landscape, not an empty route diagram. The same presentation projection drives markers, minimap and Locate. Reduce Motion and inactive/background state stop ambient animation. The older curved-geometry explanation below records the superseded presentation; ranking, consent, ghosts, finish-line and freshness rules remain current.

> **Since 2026-09-10 (deviation #72)** a scrubbable minimap sits on the right edge: the whole flight in miniature, every bird as a dot, the ridge as a tick, and a window over the part of the flight the screen is showing. A touch or drag on it scrolls the flight. The corridor is painted by your own steps — the segments behind your bird take the accent, the ones ahead stay air. `minimap.ts` owns the arithmetic and is tested against the same `flightFrame` the corridor is drawn with.
...
- **A player alone on the corridor gets a reading, not a hole (2026-09-02).** The Flock board was explicitly rewritten to stop being a picture of loneliness; the Sky had the same problem at greater scale and had not been looked at since the corridor replaced the lanes — four screens of empty gradient with no copy and no offer on it. The corridor still draws, because **the ridge is a real opponent**: it is `RACE_FINISH_LINE`, which is `DAILY_STEP_BASELINE`, flat and permanent for everybody, so a day alone is a complete reading. The observation comes first and the offer second, so the screen is about today rather than about what is missing. It **never names a rank** — there is nobody to be ahead of, and the app must not congratulate somebody for beating nobody — and **never invents a rival**. Ghost racing from scored history is unchanged and counts as a race, so this only appears to a player with nothing on the corridor but themselves. The decision and the copy live in `src/features/squad/sky-empty.ts`, tested in Node, because the screen is a component file the runner cannot load; the screen already computed *whether a race exists* to gate `race_seen`, and that condition moved with it. The offer follows what the player can actually do: somebody already in a squad shares its invite through the Flock tab's own `shareInvite`, so the message, the link and the code cannot fork; somebody with no squad is sent to where a squad is made, because "invite a friend" with nothing to invite them to would be one more surface promising something absent. The Flock tab's *presentation* is deliberately not reused — that is a card in a list of cards, and this is a gradient with a bird on it.
...
> **Since 2026-09-11** the flock first reads as a compact pastel gathering and then as the board, so the ranking reaches the initial viewport sooner. The one trailing invite, Today/Yesterday connection, leader/self distinction, private totals, solo/no-rank sentence and bird detail action are unchanged.
...
**A squad of one reads the Sky's sentence, not a standing** (issue #26, 2026-09-07). The band answered "1st · of 1 · leading" over a squad with nobody in it but you — on the tab immediately next to the one that tells the same player *"You have the sky to yourself. The ridge is the opponent"*, the app refusing to flatter them on one screen and doing exactly that on the next. The leader line beside it had been guarded on two or more rows since it shipped; the standing was a **second, separate sentence** that was never given the same guard. `resolveSquadStanding` now answers `alone` before it computes anything, and the band renders `SOLO_SKY_OBSERVATION` — the Sky's own string, imported, so the two readings of a day alone cannot drift. The size of the squad decides it, never the board: the RPC left-joins `daily_scores`, so an unmoved member is still a row and an empty board is still a squad of one. A squad of two or more is untouched. **The rows beneath the band were saying it too**, and stopped: `LeaderboardRow` takes `ranked`, false on a board of one, which withholds the rank glyph and the "Rank 1" the row label spoke — `SoloBoard` passes it as well, since its own doc already argued there was no "1st of 1" to draw there while the row drew the 1 anyway. The words themselves moved out of `Leaderboard.tsx` into `standing.ts` in the same pass — root Vitest cannot load a component file, so a rule about what a screen may say has no guard on it while it lives in a `.tsx`.
...
- **Inviting is an action, not a code to read aloud.** The invite card has a Share row, and every empty seat is itself the button — the `+` disc and "Invite your squad" were already drawn and did nothing, which is where the social loop stopped. `shareInvite()` uses React Native's own `Share`, deliberately with no clipboard dependency: the iOS sheet already contains Copy, and it puts Messenger and Viber one tap from the code, which matters more here than a "copied" toast. The message names the squad and says what Kairo is, because the recipient is not a user yet — **since 2026-08-17 by naming what they are being asked into** ("we keep each other to a daily walk") rather than describing the leaderboard to somebody with no reason to care about it yet. Every word is spent against a hard budget: the link and the code line are 84 characters between them and a test holds the whole message under 200, so adding a clause means removing one. **Universal links landed 2026-08-17** (deviation #36). The message carries `https://kairo-teal-nine.vercel.app/join/<CODE>` *and* the bare code, because a client that mangles the link must still leave six characters to type.

  **The message stopped attempting a privacy claim on 2026-09-02.** It carried "Steps, never Health data", which was false in two ways at once — steps *are* Health data, and since the reciprocal consent gate (deviation #47) a consenting squadmate sees four daily totals rather than none — and had no subject, so it asserted nothing about who sees what. The clause was justified on the premise that this message is the only Kairo copy a non-user reads, and the landing page has since made that false. **The compression is the cause rather than the symptom**, which is the argument against replacing it with a shorter true clause: the next one would fail the same way. The claim now lives on the page the link opens, stated in full — squadmates see your daily totals (steps, distance, calories, sleep) and only where you have both agreed, and never your heart rate, your workouts, or when in the day you moved. The accepted cost is that somebody who never taps the link reads no privacy claim before installing; the HealthKit permission sheet, which nobody who installs can avoid, states it correctly. `src/features/privacy/claim-surfaces.test.ts` now owns the claim across **every** surface, this message included — it is the direct answer to how the claim went stale in four places at once, and without it the next move of the consent model does the same thing again. (It owned it across the message and the page alone until 2026-09-07; three scans of one rule is itself how a surface ends up unguarded.)

  **An invited person can now arrive even if they had no app when they tapped (2026-09-02).** This was the only path in the product where somebody who *wanted* to join could silently fail to: the page told them their code would be waiting in the app, and it never was — the code is only ever captured by a route *inside* the app, and nothing carried it across an install. The landing page is restructured around the recipient instead. Their code leads, directly under the hero, with the fact that re-tapping the link after installing fills it in; the product explanation follows; the privacy statement is promoted out of the three-bullet list, because it is not one feature among three but the whole privacy promise for the invite flow and the sentence most likely to decide whether a stranger installs. The code block ships **hidden** and is revealed by an inline script that reads the address and validates six characters first — no build step and no external request, both of which are properties of that page worth keeping — so a visitor with no code, a mangled address, a crawler and a browser with scripting off all get the page unchanged rather than an empty box or garbage in a box that looks authoritative. That validation is a **necessary second copy** of the app's rule, since standalone HTML cannot import it; the guard catches its deletion, not its divergence, and that limit is recorded rather than hidden. **Order of deployment matters and the window is silent:** the page ships *before* the update that drops the message's clause, or the claim exists on no surface a non-user can reach. **Deferred deep linking is deliberately not built** — no pasteboard read (it raises a system prompt) and no attribution service (a native dependency, costing a build and withholding OTA until it lands), for a problem an inline script solves.

**Guessing a code is bounded, as of 2026-09-08** (issue #34). Six characters from `[A-Z0-9]` is a 1-in-2.18-billion shot per try, and until now nothing limited how many tries one account could take; the payoff is joining a stranger's flock, consenting, and reading their four daily totals. `rate_limits` is a per-account counter with a daily (UTC) window, charged inside the RPC by `consume_rate_limit(action, limit)` — built with `send_whack` as its second caller rather than as a special case. Five things about it are worth knowing. **`preview_squad` shares the budget**, because it answers the same question — *does this code exist?* — and hands back the squad's name, so limiting only the join would close the door you walk through after the guessing is already done. **The window is the UTC date rather than the player's local day**, alone among everything else in Kairo (§2): `profiles.timezone` is client-writable, so a local-day window would be a reset button. **`join_squad` stopped raising `22023` and returns null instead**, which is not a preference — an exception aborts the transaction and would roll back the attempt it was charged for, so the counter could never advance on the miss path and the limit would never trip. **A refusal and a wrong code are the same answer**: one null from one return statement, no code and no message, so there is nothing for a guesser to separate them with. And **the honest path never meets it** — a clean join spends two of thirty, one preview and one join, which is the best case rather than the bound (a mistyped code previews too, and the preview query's `retry: 2` can charge three for one attempt). Thirty rather than a number closer to two because the failures are not symmetrical: thirty guesses a day against 2.18 billion is the same nothing that ten is, while a false refusal tells an honest person their correct code is wrong in the one sentence designed to give them no way to find out otherwise. What it buys is stated rather than implied — the counter is per account and accounts are cheap, so enumeration costs one account per thirty tries instead of nothing, and the door that answered the existence question in unlimited volume is closed. A floor on the identity itself (App Attest) is still owed.
...
> **Since 2026-09-11** one shared plush-eagle portrait and identity header lead the page; Settings stays directly reachable. Compact streak/shield, selectable best days and Share, growth explanation and calendar follow without a second bird or statistics wall. Settings still begins with **Appearance** — System / Light / Dark — after Permissions.

Who you are, what you have earned, and the account actions. **2e's composition, as of 2026-09-08:** the XP ring, the character's name, **"A Philippine eagle"**, "Joined August 2026 · Level 12" and the XP to the next level; then the streak card, the ability ratings (gated on `full`, moved here from the dissolved character screen with the per-stat block behind them), **"How your Kairo grows"** — an ungated static explainer that says what each stat is *for* and never what you have earned — then body metrics, notifications, quest difficulty, timezone and the account actions. Settings, body-metric soft prompt, and account actions. **Quest difficulty** lives here (deviation #50) — four chips, Automatic plus the three tiers — above Timezone because this one is a choice and that one is an observation. **Other stuff, as of 2026-09-02:** Send feedback (mail to the support address, subject pre-filled), Privacy policy (opens `/privacy` on the invite host), then Sign out and Delete account — the two reversible rows above the two irreversible ones.

- **The app says what the bird is, as of 2026-09-08** (issue #32). One line under the name: *"A Philippine eagle"*. Every character has been one since deviation #55 and no screen *printed* it — a flock row's accessible label has spoken the species since deviation #40, but nothing was ever shown — which left the product's clearest cultural claim invisible — swap the eagle for a generic owl and nothing a player could see would change, which is roughly how the 2026-09-06 evaluation panel scored cultural specificity 2/5. It is **one line, on one screen**, and deliberately not a species readout, a fact card or a second noun for the character: the registry's endemic-fact `blurb` belonged to a picker that no longer exists. The words are `speciesLine()` in `src/features/character/species.ts` and a test sweeps `app/` and `src/` for the phrase, failing any file but that one — so a second surface has to read the function rather than write the sentence again, and the App Store description (`docs/app-store-listing.md`) is held to the same words. It takes the quiet body register, the only unbolded type on the screen, because it states a fact rather than making a claim.

- **The shield sentence says what is true, as of 2026-09-07** (issue #26). The card read `shield_available_on === null` as "a shield is banked" and promised *"one missed day is safe"* — but that column means only that none is *recharging*, and it is null from the first scored day, while `advanceStreak` additionally requires `SHIELD_MINIMUM_STREAK` (5) before a shield catches anything. So the first promise a new account read on this tab was false for the first four days of every account's life, on the one mechanic whose entire value is being believed *before* the day it is needed. `shieldNote` in `src/features/profile/shield-note.ts` owns both halves of the eligibility now and says "Shield unlocks at a 5-day streak" below the bar, with the figure derived from the constant rather than written as a literal. **A pending recharge is named first, at any streak length**: it is the binding constraint and was never the false half — a spent shield catches nothing however long the streak grows, and a streak that then breaks reaches five days again a fortnight before the charge returns. The pill's colour reads the same decision as its words, so the two cannot disagree.
...
whose source is allowlisted **and** which carries heart-rate evidence counts
towards Body. Both conditions, deliberately: too much is at stake to hand to an
unverified claim, and the accepted cost is that a real workout from an obscure
app that records no heart rate counts for nothing and the UI cannot yet explain
why. *(Those minutes lowered Body's bands until 2026-08-29 and raise Body's raw
value now, at `STRENGTH_MINUTE_KCAL_CREDIT` per minute — the same evidence, a
different mechanism.)*
