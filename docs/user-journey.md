...
- **Four tabs — Today · Sky · Flock · You**, flat, no raised disc, no character tab. **The app follows the phone's appearance** (deviation #72, 2026-09-10): a dark and a light scheme under one set of token roles, Settings → Appearance to override, and `userInterfaceStyle: 'automatic'` — a native field, so this pass ships with a build.
- **Today is a dashboard** (deviation #72, over #59): the date and the character's name with the Level and Streak chips, the KAIRO scene at card size, the bird's one sentence and **See today's details**, then Motion as the hero tile with the walk's meter, Body and Mind two across, and the three quests as rows. #59's rules stand: raw units, no score total, no race copy, no Mastery, the quest contract untouched.
- **The Sky has a minimap on its right edge** (deviation #72): the whole flight, every bird, the ridge, and a window that follows the scroll; drag it to scroll the flight. The corridor's segments behind your own bird are painted in the accent.
...
> **Superseded 2026-09-10 (deviation #72).** Today is a dashboard now: the date and name with Level/Streak chips, the scene at card size, the bird's sentence and the details link, Motion as the hero tile with the walk's meter, Body and Mind two across, and the three quests as rows. Every rule below about the figure, the quests, the reaction and the details sheet still holds; what changed is that the day's readings are on the page rather than one tap away. The tile sentences are composed in `today-board.ts` and tested there.
...
> **Since 2026-09-10 (deviation #72)** the violet-into-pink band is gone: the name, member count, program tag, standing, flock strip, leader line and the Today/Yesterday control all sit on the page in the page's own inks, the leader's row carries a gold rule down its edge rather than a tint, and the toggle is a `SegmentedControl` whose selected half is a raised surface rather than an orange fill.
...
> **Since 2026-09-10 (deviation #72)** the sky band behind the ring is gone and the ring sits beside the name, species line, meta line and XP rather than above them; the streak is a surface card with its flame beside the figure. Settings gained **Appearance** — System / Light / Dark — as its first group after Permissions.
