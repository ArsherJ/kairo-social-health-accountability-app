...
**The run is declared once, in `src/features/onboarding/beats.ts` (2026-09-04).** Each beat used to hand-write its own rail position — `filled={2} partial={0.5}` and so on across seven render sites, arithmetic restated seven times against a shape nothing declared. The registry declares each beat's *phase* instead and derives `filled` and `partial` from it, so adding a beat is one entry and the partials around it re-derive themselves. **The rail still measures four phases, not screens** — what this is, letting it in, your choices, the name — and each beat draws exactly what it drew before, including the name beat, whose lone phase filling completely paints the same four full segments its old `filled={4}` did. The registry also owns each beat's **button words**, which are now its own: *Let's fly*, *I'm in*, *Show me*, *Connect Apple Health*, *Lock it in*, *Good to know*, *Say hello*. Three identical "Next" taps told a new account nothing about where they were, on the one run whose whole job is orientation. `beat-registry.test.ts` scans both directories and fails any screen that puts a literal rail position or a hardcoded CTA label back, that hand-writes where Skip lands, or that reports its impression itself.
