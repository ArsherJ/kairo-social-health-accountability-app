...
### One push a day, in the morning (deviations #52, #60)
...
**Until 2026-09-02, no solo player could receive that sentence** (deviation
#60). The notification ask fired only once the account had a squad or a live
Battle, and Kairo is solo-first: most new accounts are alone for their first
days, so the one scheduled push aimed at the loop reached nobody the loop was
rebuilt for. The ask gains the account's own **first scored day** as an equal
third reason — the earliest moment the digest has anything true to report, and a
why the player can see rather than one borrowed from a squad. The two social
