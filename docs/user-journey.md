...
`app/(onboard)/connect.tsx` → `app/(onboard)/name.tsx`. **Two screens since 2026-08-27** (deviation #55): the species picker sat between them and is retired. The spec's ordering principle, and the reason for it: iOS gives one clean shot at the HealthKit and notification prompts, so stacking every permission ask before any fun front-loads friction that costs signups.
...
**`core` keeps one loop**, all of it on the Today tab: the bird in its sky, its level and streak, the day's steps in real units, the sentence naming the gap to the bird ahead, the sync line, **three quests**, the Daily Walk with its streak, and a link to *How progress works*. The Today tab says what is coming — *"Two more active days and challenges and your full stat breakdown open up."* — because an empty space where a card used to be reads as a missing feature.
...
> **Interim, until plan 3 lands.** The tab set, the palette and the Today, You
> and Flock bodies are final as of 2026-08-27. `sky.tsx` still renders the
> moved `RaceTrack` until plan 3 redraws it as the corridor — and the
> `race_seen` marker fires **nowhere** until that lands, which is a deliberate
> gap: it measures looking at the race, and the screen that used to fire it no
> longer shows one.

### Today (`index.tsx`)
The bird, the day, and what the bird says about it. The character tab and the old Today tab **merged here on 2026-08-27**: deviation #50 split them because the hero had a different subject from the list below it, and the redesign resolves that the other way, by making the day *about* the character. In order: the bird in its sky, the level and streak pills, the day's steps at display size, the sentence, the sleep and lane cards, the sync line, three quests, the Daily Walk, the Challenge door. **Every query on it was already mounted by one of the two screens, on the same key**, so the merge adds no request and cannot disagree with Sky or Flock in one frame.
...
- **The hero number opening the shelf is the day in real units, not the score, as of 2026-08-15** (roadmap deviation #30). It used to be `daily_scores.total` — a four-digit integer with no unit, no label and no target — and now reads as the day's steps at display size with the unit beside it. The rest of the ledger — distance, calories, active minutes — went with `TodayPanel` on 2026-08-27; Strain and Sleep became the bird's two observation cards, which keep the `full` gate the rows had. The engine still computes the total — it still ranks the board and feeds XP and the ability ratings — it is simply not something a first-time user was ever able to read. Anyone curious what actually ranks them can still find out, on `app/progress.tsx`'s "Daily score" entry.
...
Two mechanics, deliberately different in kind, reached from the Today tab.

**The Daily Walk** sits on the Today tab (`DailyWalkCard`) — it was on the home shelf until deviation #50 moved it, unchanged in every other respect: 10,000 steps,
...
precisely because a spread day lowers scoring Motion's Gold and must not lower a
