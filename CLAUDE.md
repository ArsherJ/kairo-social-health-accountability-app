...
- **`reject_mutation()` and the `kairo.allow_purge` flag are inert.** They enforced append-only on `sabotage_events`, which is dropped; the flag is still set by `handle_profile_deletion()` / `leave_squad()` and now guards nothing. Left in place on purpose — it is not worth reopening that path for a no-op. See `20260809120000_remove_sabotage.sql`. **Correction (2026-08-11):** that migration's comment and this line both used to say `delete_account()`. **There is no such function, and no account-deletion UI in the client** — verified against the live project (`leave_squad` is the only routine of its kind; `handle_profile_deletion` is a trigger on `profiles`). Erasure today means deleting the `auth.users` row out of band, which cascades to `profiles` and onward. Anything describing account deletion as built is aspirational — see the Phase 9 follow-up in `docs/roadmap.md`.
