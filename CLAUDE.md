...
- **TanStack Query does not know what offline means on a phone unless told.** Its default online detection is the browser's `online`/`offline` events, which React Native does not have — so without wiring it believes it is permanently online, and a query fired with no signal spends `retry: 2` immediately and lands in an error state instead of pausing. `src/lib/query-client.ts` now wires `onlineManager` to NetInfo, preferring `isInternetReachable` over `isConnected` (a captive-portal wifi is "connected" and cannot reach Supabase). The `profile-error` cover in `app/_layout.tsx` and the character screen's retry were both absorbing this.
...
- **`aps-environment` is a committed-`ios/` value that EAS would otherwise own.** Expo's notifications plugin defaults it to `development` (the APNs sandbox) and **EAS Build is what rewrites it to `production`** for a distribution build. Xcode Cloud does not — it ships the committed `ios/` as it finds it — so `app.config.ts` now declares `['expo-notifications', { mode: 'production' }]` explicitly rather than letting the default arrive unannounced. Do not treat this as proof push works: Expo's service relays to both environments. `NotificationSettingsCard` reads the value back off the running device via `expo-application`, which is the only way to compare what shipped against what was intended, and it ships in **Release** on purpose — `__DEV__` would hide it from TestFlight, the one install where the question has an answer.
