...
**Onboarding and disclosure (deviations #22, #37–#39, #58, #63, #64).** Seven beats, declared once in `src/features/onboarding/beats.ts`; the profile row commits exactly once, on `/name` — add steps **before** it, never after. `disclosureStage()` gates on lifetime `total > 0` scored days; hide on `stage`, navigate on `resolved && stage`. `syncStatus`'s `'no-data'` never shadows `'failed'`/`'stale'` and waits `QUIET_GRACE_MS`. Connecting Health is `connect-health.ts`, never inlined. Every bounded sheet: `maxHeight`, a `ScrollView` that is `flexGrow: 0, flexShrink: 1`, content in a `View` with an explicit point width; find the failure at XXXL and relaunch after changing content size. Native modals lease `src/ui/modal-owner.ts` — claim and release in the same effect.
